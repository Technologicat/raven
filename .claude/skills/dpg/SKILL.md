...
| decide whether a mouse event belongs to your widget — a global wheel/click handler, a drag, "is the pointer over this panel" | *Drawlists* → "Is this mouse event mine?" is `is_item_hovered`, and a rectangle cannot answer it (`guiutils.is_mouse_on_widget`; an "is a modal up?" guard is the wrong question) |
