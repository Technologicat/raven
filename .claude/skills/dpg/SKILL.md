...
| call `split_frame` anywhere at all | *Threading* → `split_frame()` mechanism, **and** Use `guiutils.split_frame`, not `dpg.split_frame` |
...
| diagnose a hang with no traceback, or a background-task race | *Threading* → Three-way deadlock pattern · Diagnosing background-task races |
| reach for `dpg.mutex()` | *Threading* → `dpg.mutex()` — the atomicity tool that Raven cannot currently use |
| upload, replace or delete a texture | *Threading* → Texture upload ordering · *app structure* → Textures |
| build a widget that will later be rebuilt or swapped | *app structure* → DPG item management (version-counted tags; alias rebinding) |
| set a widget's value from code | *app structure* → DPG item management (no callback fires; `configure_item` vs `set_value`) |
| add or change a hotkey | *Keyboard input* — all of it. The `mvKey_*` constants lie (517/518), and same-frame dispatch is by keycode, not press order |
| gate a hotkey on a text field having the caret | *Keyboard input* → Focus is not the same as the caret: gate on `is_item_active` — **and** the commit-chord exception |
| park focus on a panel or child window | *Keyboard input* → `focus_item` cannot focus a child window — and does harm when asked to |
| scroll programmatically, or follow a growing log | *Scrolling* — both sections |
| set a window's size, or a tooltip's padding, or fight z-order | *Window sizing* |
| touch fonts, icons, super/subscripts or `dpg_markdown` | *Font atlas limits* |
| write a test that drives DPG | *Testing DPG code* → especially the headless **ceiling**: no `render_dearpygui_frame`, so no layout |
| start a new app, or wire its startup sequence | *Raven DPG app structure* — the whole section |
