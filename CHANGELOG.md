...
- **`Page Up` and `Page Down` scroll the info panel while you are typing a search**, so its results can be read down without leaving the field. Previously the field had to be left first, which only the mouse could do without losing or accepting the search. Raven-librarian's chat log already works this way.
