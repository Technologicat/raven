...
- documents whose extracted text contains stray UTF-16 fragments — in practice, PDFs using the mathematical alphabets, so scientific papers — now index and attach instead of failing. The extracted text was passed on unrepaired and died at the first thing that had to encode it, which for the document database was the server: the ingest of that document aborted with `HTTP 400 BAD REQUEST` and nothing in the message named the file or the character. Text is now repaired at extraction, so every consumer gets valid text. A fragment that encodes a real character is restored to it (*𝐴* stays *𝐴*, rather than being deleted along with the rest of the equation); one that encodes nothing is dropped, and the log names the document either way. Indexing 1268 arXiv papers, three were silently lost this way.
