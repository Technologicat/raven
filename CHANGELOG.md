...
- **exported chat logs now mark a thinking model's reasoning**, wrapping it in `<think>`/`</think>` ahead of the reply. The two export paths previously disagreed and neither was right: copying a single message ran the trace straight into the answer with nothing between them, so a reader could not tell where the model stopped reasoning and started replying, while the full-log export dropped the trace entirely. Both now carry it, marked, from the same source — so the same message reads the same way whichever way it left the app.
