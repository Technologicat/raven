...
9. **When you edit a postprocessor filter's docstring, run `python scripts/gen_postprocessor_manual.py`** to regenerate `raven/common/video/postprocessor-filters.md`. It is *generated* from those docstrings, so it goes stale the moment one is edited, and **a generated file that is checked in looks exactly like a hand-written one** to everybody who reads it.
   - **CI catches it if you forget**, on one matrix entry of the `test` job (`--check`, which exits 1 and names the file). One entry is enough, the input being docstrings rather than anything that varies by platform; it lives in `test` rather than `lint` because it needs torch.
   - Worth knowing that torch *is* available in CI, since the requirements file suggests otherwise: `requirements-ci.txt` has none, and the workflows install the trio from PyTorch's CPU wheel index in a line of their own. This rule spent an evening saying the check could not run in CI on exactly that misreading.
