...
### Machine-local settings live outside the repository

`~/.config/raven/overrides.json`, keyed by config module, applied by `raven.configoverrides` as the last
statement of every config module. Shipped defaults stay in the tracked `config.py` files and the override
file wins. The README's *Configuration* section has the format.

**So a tracked `config.py` is an ordinary source file, and the tree is expected to be clean.** Until
2026-09-11 it was not: the three configs carried machine-local overrides as their steady state, and a long
section here forbade every bulk `git add` form in an attempt to keep them out of commits. It failed three
times anyway, because the failure needs a file somebody is *legitimately* editing, and a rule that caught
that would have blocked the feature work. A file boundary is what fixed it. The deny rules on `git add -A`,
`-u`, `.` and `raven/` stay in the agent's permission settings; they cost nothing and guard other things.

### `git stash` is a mutation of the working tree in service of a read

Independent of the above, and it survived the fix because it was never really about `config.py`.

- **`git stash push <paths>` can fail and leave you believing it worked.** An intent-to-add file elsewhere in the tree (`git add -N`, which is what a newly written and not-yet-committed script looks like) makes it abort with "Entry ... not uptodate. Cannot merge." A script that pushes, runs something, and pops then runs its check against the *unstashed* tree — measuring exactly what it meant to revert.
- **Prefer not stashing at all.** To see how the tree behaved before an edit, read it out of git instead: `git show HEAD:path/to/file`, or `git stash create` (which makes a commit and touches nothing).
- **Never `git stash pop` or `git stash apply` without naming the entry and reading it first** — `git stash show --name-only stash@{N}`, then decide. `pop` with no argument means `stash@{0}`, whatever was stashed most recently by anyone, not what this session pushed.

(Live case 2026-08-31: a `git stash push` on four named paths aborted for exactly that reason; the check that followed silently measured the already-fixed tree, and concluded the opposite of the truth.)
...
6. **Run `python scripts/check_usage_paths.py` when preparing a release**, and after moving or renaming any module that a usage line names. It checks that every `python -m raven...` in the repository — docstrings, READMEs, shell scripts — still names a module that exists. These are instructions a reader is meant to *type*, and nothing else in the toolchain looks at them: no import, no lint, no test. Nobody on this side runs them either, the console scripts being right there, so a wrong one is discovered by a user following the README literally. Release time is the trigger because that is when the documentation goes out. Four were wrong when it was written, in three different ways.

7. **Run `python scripts/check_option_lists.py` after adding a value to a list of options that prose also enumerates** — an upscaler `quality`, a hotkey. Those copies are hand-maintained, because a README and a config comment cannot import anything, so the toolchain has no other opinion about them: a value added to the code and not to the documentation is invisible to every reader who learns the options from the docs. It also **notes** the reverse — keys a README documents that the app's F1 card does not offer — which does not fail the run: a card is a fixed-height window and is knowingly the smaller document, so that note is the standing list of what is waiting for a card redesign.

8. **Run `python scripts/check_doc_links.py` after renaming a heading or adding an anchor link**, in any tracked Markdown — and **when preparing a release**, alongside rule 6, for the same reason: release is when the documentation goes out, and a link that lands nowhere is found by the reader who clicks it rather than by anyone here. A hand-maintained table of contents drifts the moment a heading changes, and nothing about editing a heading prompts anyone to revisit the TOC — so the link keeps rendering and simply lands nowhere, which only a reader who clicks it discovers. Same rot as rule 6's usage paths, and the same reason nothing else catches it. Note that whether a TOC is meant to list *every* heading is per-document, recorded in the script: the component READMEs index their top-level sections deliberately, and only the main `README.md` is checked for completeness. It is also, as of 2026-09-20, the only `scripts/` checker with tests of its own (`scripts/tests/`), which is where the others' should go.

9. **Run `python scripts/gen_postprocessor_manual.py --check` when preparing a release**, alongside rules 6 and 8 and for the same reason — release is when the documentation goes out. `raven/common/video/postprocessor-filters.md` is *generated* from the filter docstrings, so it goes stale the moment one of them is edited, and **a generated file that is checked in looks exactly like a hand-written one** to everybody who reads it. `--check` exits 1 and names the file; running the script without it regenerates.
   - **It could run in CI, and currently does not.** The first version of this rule said it could not, on the grounds that importing the postprocessor needs torch — which `requirements-ci.txt` indeed lacks, but the workflows install the torch trio from PyTorch's CPU wheel index in a line of their own, on all three OSes. `check_ci_imports.py`'s own machinery confirms every module-level import of both the postprocessor and the generator is satisfiable there. So release time is the *weaker* trigger, catching a stale file weeks after the docstring edit that staled it, and the better answer is one guarded step in the `test` job. Until that exists, this rule is what there is.
   - **A second generated document is the signal to stop and reconsider the approach**, rather than to write a third generator. Juha's observation on the day this landed (2026-09-22): we are essentially hand-rolling a partial Sphinx. That is fair, and the differences are currently load-bearing — the generator reads *runtime* metadata (`get_filters()`'s defaults and ranges, which come from `with_metadata` and appear in no docstring), it emits Markdown that GitHub renders with no build step, theme or hosting, and this project's style guide says outright that docstrings use plain backticks because "the codebase is read as raw code, not via Sphinx". Each of those reasons holds for one generator and gets weaker with every one added.
...
**This file is long enough that its entries are titled**: the bold lead on a line of its own, the prose
starting on the next one, and one line per bullet throughout. The rules and the measure they come from — a
section is as good as it is scannable — are in the `changelog` skill. What is Raven-local is only that we
crossed the size where titling starts to pay, at 139 top-level entries in 0.2.9 alone.
...
The reason it needs saying is that nothing will feel missing. The recipes in there are the ones whose absence is silent: a fixed `sleep` in place of the skill's wait-on-the-ready-line loop appears to work every time. So the moment that would prompt you to go and look never arrives, and the seven bullets below — which *are* here in full — read as the whole of what the job needs.
...
- **Announce a launch, and any input injection, before it happens — in the message *and* as a desktop toast.** The mapping window steals focus by itself, so this is owed even for a look-and-see run. Send both, because they fail in opposite directions and neither covers the other's case:
  ...
  - **The toast reaches a session the user cannot see** — `cc-toast "taking keyboard focus"`, which also makes a sound. That is the dangerous case: a window mapping out of a backgrounded session steals focus from whatever they are actually doing, and there is no output for them to be reading. Its cost is position — it appears in the far corner of the display, about as far from the composer as the screen allows.
    - **Put `cc-toast` in the same command as the launch, as its first line — never in a call before it.** What the toast tells the user is not "a launch is coming" but "the command cleared the permission check and is running *now*", and that is the instant they know to take their hands off the keyboard. Sent from an earlier call it says that too early: the launch may still be waiting to be approved, so the warning burns their attention on a window that is not yet mapping, and the useful signal arrives with nothing to mark it. (Juha, 2026-09-04, on what he actually reads it as.)
...
- **When the maintainer will judge a change by using the app, launch it yourself and hand over the
  keyboard** — with `--log` into your scratchpad and `--repl`, and without restoring focus. They drive with
  real input and you read the log as they go; don't ask which of you should launch. Recipe: the
  `live-gui-testing` skill, *Testing together*.
...
- **Never `pkill -f raven-<app>`.** The pattern matches the agent's own shell command line, so it kills the invoking shell (exit 144) and usually leaves the app running. Select real PIDs instead — `pgrep -af raven-librarian | awk '$2 ~ /python/ {print $1}' | xargs -r kill` — or better, close it through the window manager, as below.
- **Close with `wmctrl -i -c <window-id>`, and never with `xdotool windowclose`.** The recipe is here rather than in the skill because deferring it is what fails: "close it through the window manager" is exactly what `windowclose` sounds like, and by the time the app is up and wants closing, nothing has prompted the skill to load. `windowclose` calls `XDestroyWindow` — it takes the window away without asking, the toolkit's connection breaks, and the process dies with no `atexit`, which for Librarian means losing whatever the chat gained since its last periodic save (up to a minute; before 2026-09-15 it saved only at exit, and a session's chat went with it). `wmctrl -i -c` sends `_NET_CLOSE_WINDOW` and lets the app close itself. Both `xdotool windowclose` and `windowkill` are denied in the permission settings as of 2026-09-03, after this cost a session's messages twice (2026-08-26 and 2026-09-03) — the skill has said not to do it the whole time, which is why the rule is here and enforced rather than merely written down again.
  - **Check that it worked**, since the failure is silent: the log should end with the app's own shutdown lines (`app_shutdown: done` for Librarian), not simply stop. A `PersistentForest.save: All done` appears there only if the chat changed since the last periodic save — an unchanged datastore is skipped at DEBUG level — so its absence is not a lost save; compare `chat.json`'s mtime with the last change instead (found 2026-09-17).
- **A screen coordinate is `xwininfo`'s *Absolute upper-left* plus the coordinate read off the screenshot. Never `xdotool getwindowgeometry`**, whose `Position` adds the client's offset inside its frame a second time — so every click lands one decoration-height too low and silently hits nothing. Verified again 2026-09-07: `xwininfo` said the window was at `(0, 100)`, `getwindowgeometry` said `(10, 140)`.
  - **And a click needs to be held across a frame.** `xdotool ... click 1` presses and releases in about 12 ms; ImGui samples the button per frame, and an idle-throttled app renders at ~12 fps, so the whole click falls between two frames and is never seen. Move, `sleep 0.4` so the app has the new pointer position, then `mousedown` / `sleep 0.3` / `mouseup`. Same mechanism as the synthetic-key rules in the skill, on the other device.
  - **Hover is the free instrument for both**, and worth using before believing any negative: DPG highlights a widget under the pointer, so a screenshot after a bare `mousemove` says whether the arithmetic is right, with the click question still untouched.

**Why these seven stay and the recipes go.** The costs are asymmetric: forgetting most recipes wastes a run of your own, while forgetting to announce spends the human's keystrokes on whatever the app makes of them. A rule that protects *someone else* cannot be demand-loaded, because the moment it is needed is the moment before the task has been recognised as the kind that needs it.

That is also why the *closing* recipe is here while the rest went to the skill: getting it wrong destroys the human's data rather than your afternoon, which puts it on this side of the line by the same argument. The test for a recipe is not how detailed it is but who pays when it is forgotten.
...
9. **Writing `dpg.create_context()` is the trigger to stop and check — the token, because the tasks that reach it never present as DPG work.** "Give this test its own context" reads as test plumbing. The measured hazard is recreation *once real widgets have rendered*: two contexts per process with a `FileDialog` built in each died with `SIGSEGV` in **3/8 to 8/8 of runs**, nondeterministically, mechanism unidentified — and the cost is not ordering-dependent, so run early such a test takes down the next context created anywhere in the process (including unmapped ones, in an unrelated module) and run late it dies in its own `setup_dearpygui`. What keeps the default suite clear is that it never renders a frame; about half its DPG modules do hold a *function*-scoped context and are green. So the rule is not "one context per process" but: **a module takes its context from its own fixture, and nothing creates a second one beside it.** Anything that maps a viewport and renders shares the session-scoped `mapped_gui_context` in the root `conftest.py`; `test_filedrop.py`'s lifecycle tests are skipped whenever a shared-context test is collected, because they cannot coexist with one. A green run proves nothing either way, the crash being nondeterministic and landing elsewhere. **And check what the context was for**: reaching for one to escape a tag collision is the common false lead, where version-counted tags (pitfall 5) or simply not colliding is the real fix. `dpg-notes.md` → "Context recreation is not reliably safe once real widgets have rendered" has the measurements, and the eleven days the `--run-gui` group spent broken by one function-scoped mapped fixture.
...
- **User-facing text is grammatical English, articles included** — *"Start a new chat"*, not *"Start new chat"*. Instrument-panel terseness is a concession that buys horizontal space, so spend it only where space is actually short. A UI element's own *name* (the *Thinking* / *Internet* / *Documents* toggles) is exempt: that is what the control is called. Full rules, including what a help-card row owes a reader who meets it before the prose, in `raven-style-guide.md` under "User-facing text".
...
- **As little global state as possible — except for appstate-category things** (Juha, 2026-09-22). Application state is the exception and a real one: the Visualizer keeps `dataset`, `selection_data_idxs_box` and the rest at module level on purpose, and its `CLAUDE.md` says so. What the rule is against is *incidental* globals — a shared component reaching for a module-level handle because plumbing a parameter through its callers looked tedious.
  - **So a shared component takes what it needs as a parameter**, even where the thing is process-global in fact. `helpcard.HelpWindow` takes `themes_and_fonts`; so does `FileDialog`, across sixteen call sites, rather than reading fonts `bootup` could have left somewhere. DPG really does have one font registry per process, so a module-level handle would have been *correct* and is still the wrong shape: it hides a dependency that a signature states.
  - The failure this prevents is not a wrong answer but an unanswerable question — with a global, "what does this widget need in order to work?" has no answer short of reading its body, and every future shared widget inherits the same shortcut.
...
- **Behaviour that lands in `app.py` is untested behaviour.** Importing a Raven `app.py` **runs that app** — there is no `main()` and no `__main__` guard, so the module body *is* the program: it parses the command line, builds the GUI and enters the render loop, all at import time. Failing on pytest's own argv is only the first thing that goes wrong, and feeding it a plausible argv would get further into starting an app rather than closer to a test. That is a fine property for an entry point and a fatal one for anything else: a function put there cannot be exercised at all. `raven.cherrypick.preload.donate_outgoing_image` lives where it does for this reason.
  - **Whether they should have a `main()` is a separate question, and DPG argues against it twice** (Juha, 2026-09-10). Noted so the next reader meets the obstacles rather than proposing the wrapper as an obvious win.
    - **Module scope is doing work during GUI setup**: it is where the widget references are kept, and the callbacks that later use them are at module scope too. Move the setup into a function and those become locals, so every callback needs `global` or a closure over them — a restructuring rather than a wrapper.
    - **And a `main()` would advertise a re-entrancy a DPG app cannot have.** Call it twice and setup rebuilds widgets under tags that already exist, which is pitfall 5 — a duplicate tag **takes the process down** rather than raising, so the mistake costs a crash with no traceback rather than an error anyone can read. A module body cannot be run twice by accident, the import machinery running it once and caching it, so the current shape makes the error unavailable rather than merely unlikely.
    - Neither is what the rule above rests on: even with a `main()`, an entry module is where the wiring goes, and wiring is not what wants testing.
