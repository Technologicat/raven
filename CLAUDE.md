...
8. **Run `python scripts/check_doc_links.py` after renaming a heading or adding an anchor link**, in any tracked Markdown. A hand-maintained table of contents drifts the moment a heading changes, and nothing about editing a heading prompts anyone to revisit the TOC — so the link keeps rendering and simply lands nowhere, which only a reader who clicks it discovers. Same rot as rule 6's usage paths, and the same reason nothing else catches it. Note that whether a TOC is meant to list *every* heading is per-document, recorded in the script: the component READMEs index their top-level sections deliberately, and only the main `README.md` is checked for completeness. It is also, as of 2026-09-20, the only `scripts/` checker with tests of its own (`scripts/tests/`), which is where the others' should go.
