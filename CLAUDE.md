...
7. **Focus is not the caret, and `focus_item` cannot focus a child window.** ImGui auto-focuses the first navigable item of a window, so a text field reports `is_item_focused` True within a few frames of startup with nobody having touched it — a global hotkey handler gated on that silently swallows every key it delegates to the field. Gate on `dpg.is_item_active` instead, which is True only while the field owns the caret.
   - **Except for Enter, where the right predicate depends on the field's kind.** A *single-line* `InputText` deactivates itself on Enter (the key commits the edit), so an Enter handler gated on `is_item_active` can never fire — gate that one on `is_item_focused`. A *multiline* field stays active, since there Enter inserts a newline. Hence `raven-visualizer` (single-line search field) and `raven-librarian` (multiline composer) legitimately gate their Enter handlers differently, while both gate their bare-key branches on `is_item_active`. Getting this wrong kills the hotkey silently: it was shipped and only caught in live testing.
