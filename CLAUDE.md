...
5. **DPG callback thread — single dedicated thread, not the main thread.** DPG dispatches event callbacks (mouse/key handlers from `handler_registry`) and frame callbacks on a single internal thread, separate from the main thread that calls `render_dearpygui_frame()`. Only one such thread exists globally. This means callbacks run concurrently with the main loop, and a lock held by a callback blocks the main thread if the main loop tries to acquire the same lock. **Deadlock pattern:** callback holds lock L → calls `TaskManager.clear(wait=True)` on a task that uses `split_frame()` → task waits for `render_dearpygui_frame()` to complete a frame → main loop tries to acquire lock L before reaching `render_dearpygui_frame()` → three-way deadlock. **Rule:** never call blocking waits on `split_frame`-using tasks while holding locks that the main loop needs. Prefer deferring heavy work (image loading, task cancellation) to the main loop body via a pending flag.
