...
- **Synthetic input does need focus.** GLFW ignores the `XSendEvent`-based `xdotool key --window <id>`, so driving the GUI means really activating the window. When doing that: **say so first**, keep the burst short, and **restore focus afterwards** — capture `PREV=$(xdotool getactivewindow)` *before* launching or activating, and `xdotool windowactivate --sync "$PREV"` when done.
  - **A synthetic key is not a keypress. Read `dpg-notes.md` → "Keyboard input" → "`is_key_down` is sampled when the callback runs, not when the key was pressed" before sending one** — it carries the fidelity traps, which are the half this section does not. In short: `xdotool key ctrl+b` arrives with its modifier already released and has to be sent as `keydown` / `key` / `keyup`; a held modifier auto-repeats as press events alongside a pseudo-key no `mvKey_*` constant names; and a synthetic tap is about 12 ms where a finger is ten times that, which hides anything depending on how long a key is held.
  - **The failure they cause is a *finding*, not a crash**, which is what makes the pointer worth its line: the app looks like it ignored the chord, and that reads as a bug in the app rather than in the harness — so the run gets written up instead of retried. (Live case 2026-08-21: both traps hit again while diagnosing a real Tab bug, having been documented since 2026-08-17. The notes were right, and neither document pointed at the other.)
- **The whole drive sequence goes in ONE Bash call.** Activate → click → type → send → restore focus must be a single command. Each Bash call raises its own permission prompt, and the prompt takes focus back to the Claude Code window; the human then acknowledges it, which *re-focuses CC*, and the `xdotool` in the next call drives whatever is focused now — the terminal, not the app. Splitting "press Enter" into its own call is enough to lose the keystroke to CC.
  - **The failure is silent and looks like the app ignoring input.** Nothing errors: `xdotool` reports success, having typed into the wrong window. The tell is a screenshot that is byte-identical to the one before, and a log with no new lines. Verify by capture, never by "the command exited 0".
  - Anything that must be waited for goes in the same call too (a bounded `for i in $(seq …); do grep -q … && break; sleep 2; done`), so the wait does not become another prompt.
  - **A failed window lookup wedges the sequence, and `--sync` is why.** `xdotool search` printing nothing leaves the variable empty, and `xdotool windowactivate --sync ""` then *blocks* waiting for an activation that will never happen, until the Bash timeout kills the whole call — taking the app it launched with it, since that is the same process group. Guard with `[ -n "$WID" ] || exit 1` before the first injection, so the failure is a message instead of a two-minute hang.
    - **`xdotool search` has no `-i`, and needs none**: `--name` takes a regexp and matches case-insensitively (measured 2026-08-19 — an all-caps pattern returns the same window id as the exact-case one). Passing `-i` anyway is how the lookup comes back empty in the first place. Worth knowing because the app titles are not consistent about it: `raven-cherrypick` is lowercase, `Raven-librarian` and `Raven-visualizer` are not, and the xdot viewer is `Raven XDot Viewer`. A pattern of `raven` finds any of them; add `--onlyvisible` so a stale or unmapped window cannot answer instead.
- **Focus the window, then click the field.** A window can be focused while no widget owns the caret — DPG's own pitfall #7, met in the wild here: `windowactivate --sync` succeeded, `getwindowfocus` returned the right window, and every keystroke still went nowhere because the composer was merely *focused*, not *active*. Click the target field first, then type. (`raven-librarian`'s composer also has a `Ctrl+Space` hotkey for this, which its tooltip advertises.)
...
- **Restoring focus also *raises* that window, burying the app under test.** `xdotool windowactivate` focuses and raises together, so the tidy "give the human their window back" step at the end of a drive sequence puts Claude Code on top of the app that was just launched — which reads as the app having closed. Cost a false crash investigation once (process alive, window present, simply behind). Either say where the window went, or skip the restore when the human is about to drive the app themselves, which is the common case after a restart.
...
- **Get click coordinates from `xwininfo -id <wid>`, not from `xdotool getwindowgeometry`.** A screenshot taken with `import -window <id>` is in *client-area* coordinates, so `screen = xwininfo "Absolute upper-left X/Y" + the coordinate read off the screenshot`.
  - **The in-app half of that sum is `guiutils.get_widget_pos(widget)`**, which reports a viewport position for anything — including the windows and child windows that have no `rect_min` — so a click can be aimed at a widget by name instead of by eye. `screen = xwininfo origin + get_widget_pos`, the viewport's client area being what the screenshot shows. Reading a coordinate off a screenshot still works and needs no running Python, but it has to be re-read whenever the layout moves.
  - **In a throwaway probe, `dpg.set_primary_window(win, True)` is the cheaper trick**: it drops the title bar and pins the window to the viewport origin, so the arithmetic is the widget's own offset and nothing else. Worth knowing because the failure without it is silent — a click 25 px too high lands on the title bar and starts a window drag, and the probe then reports that typing went nowhere.
