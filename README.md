...
Every GUI app accepts files dropped on it from your file manager. **What a drop means is whatever that app's open button already means**, so there is nothing new to learn per app:
...
| | one or more `.bib` files | opens the importer dialog, with those files already filled in as inputs |
...
The settings editor has two image slots and a drag cannot be aimed at either — due to technical limitations, the drop event only materializes when the mouse button is released, so we cannot highlight the target that would accept a drop being hovered — so the image decides: a character is a cutout, a backdrop is a full frame.

Drop something an app cannot use and it says so, naming what you dropped and what would have worked, rather than doing nothing. A drop that arrives while a dialog is open is ignored.

The drop feature works wherever the GUI toolkit's own windowing layer does: X11, macOS and Windows. **Wayland is untested** — please report if it does not work there.
...
- **`--server-url URL`** points the tool at a *Raven-server* other than the configured one, and **`--backend-url URL`** at a different LLM backend. Each is logged against the configured value it replaced.
  - Handy for a one-off run against another machine without editing any settings.
...
- **`--repl [PORT]`** opens an in-process REPL inside a running app, off unless asked for. On every GUI app, `raven-minichat` and `raven-server`.
...
- `raven.config` → [`raven/config.py`](raven/config.py)
  ...
  - `CALIBRATION_FPS` is the reference clock everything animated is *tuned* against, and is **25**. It is not a speed Raven runs at — every animation is corrected at run time to whatever frame rate the machine actually achieves, so a slow machine shows a smoother-or-choppier version of the same motion at the same wall-clock speed, never a slower one.
    - What it *is* is the unit the numbers beside it are quoted in. The avatar's `pose_interpolator_step` is the fraction of the remaining distance covered in one frame at 25; its `blink_probability` is per frame at 25; a video postprocessor effect's durations are in frames at 25. Those settings are documented in their own files and read in this unit.
    - **Changing it is coherent but expensive.** Nothing assumes the number — every use is a ratio — so a different value is self-consistent. But it redefines the unit, so every quantity that was tuned against it is then wrong by the same factor and has to be re-tuned by eye. It is written down so the assumption is findable, not because it is a dial worth turning.
...
Settings that have no JSON form at all, such as a function, are simply not overridable this way — Raven says so in the log and keeps the shipped value — and neither is a setting Raven *derives* from another, which has to be overridden in its own right. Those stay `config.py` edits.
