...
**The two files are for different things, which is why this is a split rather than a replacement.** `config.py` stays the place the *documentation* lives — what each setting means, what the other values are, why one was chosen — and it is also a Python file, so it can compute things: a directory built from another directory, a toolbar height from a font size, a setting whose value is a `torch` data type and has no JSON spelling at all. None of that can move. Your overrides are the other half: short, specific to this machine, and needing no explanation beyond the occasional note. So read `config.py` to find out what you can set, and write `overrides.json` to set it.
