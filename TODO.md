...
- **[Medium]** Close the arXiv metadata workflow inside the constellation: `raven-arxiv2id` extracts identifiers from a directory of PDFs, but nothing in Raven turns those identifiers into BibTeX, so the documented path runs through the external `arxiv2bib`. Noticed 2026-08-06 while building a retrieval corpus, when the tool turned out not to be installed on this machine at all — an omission nobody could have noticed, because the two-step workflow is not written down anywhere in the repo either.
  - **The external tool has no rate limiting** — checked its source: no sleep, no delay, no throttle. Raven's own `raven-arxiv-search` and `raven-arxiv-download` both use `papers.ratelimit.RateLimiter` at arXiv's documented 3 s. So the current workflow is polite at both ends and rude in the middle, which is the sort of thing that gets an IP blocked at the scale this is for (1268 identifiers).
  - **Everything needed is already in the tree.** The arXiv API endpoint `raven-arxiv-search` calls takes an `id_list` parameter alongside `search_query`, and `httpfetch.arxiv_get`, the `RateLimiter`, the feedparser handling and `bibtex.entries_to_bibtex` are all in place. This is a new entry point over existing parts, not new machinery.
  - Open design question, which is why this is not just done: a separate `raven-arxiv2bib`, or a flag on `raven-arxiv-search` (it already emits BibTeX; this is the same operation with a different selector), or an option on `raven-arxiv2id` so the whole PDFs-to-BibTeX path is one command. The third reads best from a user's point of view and worst from a separation-of-concerns one.
  - Either way, **document the workflow** — that gap is independent of which shape wins.
