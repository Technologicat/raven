...
- *RAG reranking* — **now off the list entirely: measured and rejected** (2026-08-06, see the struck item
  ...
  - The **query-side levers** (`briefs/librarian-extension/done/09_retrieval-query-side-brief.md`) were argued onto the demo path
...
- **[Done 2026-08-26, except the mid-thought resume]** Thinking toggle: let the user turn a thinking model's reasoning on and off. **Shipped as the *Thinking* checkbox**, implemented as `reasoning_effort: "none"` and nothing else — `llmclient.thinking_request_fields` is the whole mapping, `invoke` and `scaffold.ai_turn` take a `thinking_enabled` parameter, and `appstate` carries the flag. None of the prefill machinery the July design called for was needed.

  **What is left of this item is the mid-thought resume**, which is a different job with a different mechanism (see below), and the measurement record, which the researcher's-night brief's step 8 cites for exactly that. Everything below is kept for those two reasons rather than as pending work.

  Researched 2026-07-27 — brief 02 closed the *transport* half of this item but none of the request-side half, so the remaining work is narrower and differently shaped than originally written:
  ...
  - **Probed against LM Studio 2026-07-27; the obvious mechanism does not work and the rejected one does.** Measured on `qwen3.5-9b` and `qwen3.6-35b-a3b`:
    ...
    - **Settled 2026-08-26 (Juha), and it is two cases rather than one.** Continuing a reply that was cut off *mid-answer*, its thinking already done, must not open a new thinking block — Continue does what it says on the tin, and that is what happens today. So the toggle needs no exception for it.
      - **Continuing a reply cut off *mid-thought* is a different case, and today's behaviour there is a defect** (Juha): "continue" ought to mean *pick up where you left off, here is what you already produced*, and a backend that cannot is wrong rather than merely limited. Possibly all of them are, this case simply not having come up.
      - **On Qwen it looks buildable without them, and both ingredients are already measured.** The prefill is *ours* — `invoke` serializes the partial reply as a trailing assistant message, and a trailing assistant message means the template emits no generation prompt, so that text lands at the end of the prompt verbatim. Put the partial reasoning inside an *unclosed* `<think>` and the prompt ends inside the block. Prior probes (2026-07-27, both on `qwen3.5-9b` and `qwen3.6-35b-a3b`): an open `<think>\n` prefill does force thinking on, and a trailing assistant message is genuinely continued rather than restarted. **What is untested is the composition** — an open block that already *contains* text — and what LM Studio's server-side reasoning parser does with a prompt that ends mid-block. Probe that before building.
      - **Do not read the thinking-history discard as ruling this out.** It is a different question, and the item below says so in passing: `preserve_thinking` and friends govern how *history* renders, not how the next turn starts. The three routes measured dead there — the `reasoning_content` sibling field, `chat_template_kwargs`, the Anthropic `thinking` block — are all ways of asking the *template* to re-emit prior reasoning. Text placed in the trailing message's `content` is not one of them.
      - **It is a Qwen mechanism, not a general one** (`TODO.md`, Gemma 4 bullet below): Gemma's template puts `<|think|>` at the top of the first *system* turn, so there is no mirror image of the trick there and thinking cannot be forced on by prefill at all. So this reintroduces per-family handling for the *resume* direction, which `reasoning_effort: "none"` had removed for the *off* direction — worth knowing before it is costed as small. What Gemma needs instead is unexamined.
      - **It needs the same signal as the parser fix**, which is the reason to build them together: something must tell `StreamParser` it is starting inside a think block, and that is exactly the `_PS_THINK` start the deferred item *Streaming thinking shows as gray* is waiting on.
      - **Unmeasured, and this is the part to probe first:** LM Studio parses reasoning server-side because it applied the template and knows the state. Handed a prompt that *ends* mid-block, its parser may or may not still track correctly. One request answers it.
  ...
  - **Re-probed 2026-08-26, and one of the July findings has to be read differently now.**
    - **`chat_template_kwargs` is still ignored** — confirmed on the current LM Studio build against `qwen3.6-35b-a3b`, whose template does read `enable_thinking`. Sending `enable_thinking: false` would select the template's non-thinking branch and produce *zero* reasoning; it produced 884 characters of it. The sharper evidence is `prompt_tokens`, identical at 26 across all four conditions: the rendered prompt never changed at all.
    - **The backend does report the reasoning/answer token split**, in `usage.completion_tokens_details.reasoning_tokens`, on the streamed usage chunk as well as the whole-response one. So the thinking readout is exact on LM Studio with no local tokenizer configured. `llmclient.thinking_token_count` reads it as its first tier.
    - **Qwen 3.8's stock template defaults `reasoning_effort` to `xhigh`** — read out of the GGUF, not inferred: `reasoning_effort|default('xhigh')`. A client that sends nothing therefore gets the *maximum*, which is the overengineering complaint. Supported values are `xhigh`, `medium` and `low`; **`high` is silently mapped to `xhigh`**, and anything else calls `raise_exception`, so a wrong value is a hard render failure rather than a fallback to a default. The instruction block it emits is 339 / 102 / 268 characters for xhigh / medium / low, which is what makes `prompt_tokens` a clean readout for the next probe.
      - **Consequence for Raven**: if the effort is to be controllable at all, the value has to be validated on our side before it is sent, and "send nothing" is not a neutral choice on this model. The current workaround is a hand-edited template override in LM Studio (Juha, 2026-08-26).
    - **`reasoning_effort` is honoured, and `"none"` is a working thinking-off switch** — measured on `qwen3.8-27b@q4_k_xl`, LM Studio **0.4.20 (build 1)**. This is the finding that may replace the prefill mechanism below, so it is the one to re-check first if any of this is revisited.
      - **LM Studio processes the field rather than passing it through.** It validates against *its own* list — `none, minimal, low, medium, high, xhigh` — and refuses anything else with an HTTP 400 naming them. That list is wider than the Qwen template's (`xhigh, medium, low`), and a value in the gap (`minimal`, `none`) does **not** reach the template's `raise_exception`. So the field is LM Studio's, not the model's.
      - **`"none"` gives `reasoning_tokens: 0`** with the answer still correct, and a visibly shorter rendered prompt (28 tokens against 56). Every other accepted value was indistinguishable from sending nothing — 56/57 reasoning tokens across `minimal`, `low`, `high`, `xhigh` and absent. So the switch is binary on this backend: thinking, or none. There is no gradation to expose.
      - **The gradation half is not a measurement of anything**, and the reason is that the model under test carries a template override which begins `{%- set reasoning_effort = 'low' %}` (Juha's, plus `preserve_thinking = false`). A Jinja `set` at the top rebinds the name for the rest of the template, so whatever the client sends is discarded before the effort branch reads it. Under that override the question cannot be asked at all. **Removing those two lines is the whole prerequisite** for probing whether LM Studio forwards the field.
      - **`"none"` works through a path the override cannot block**, which is the sharper reading of the same run: it changed the rendered prompt (28 against 56) even though the template was pinning `reasoning_effort` to a constant. So LM Studio is not implementing `none` by passing it to the effort branch — it is switching thinking off by some route of its own, and that route is a better bet for working across models than one that depends on a template variable.
      - **Do not compare local template renders to the backend's `prompt_tokens` in absolute terms.** An earlier note here did, and it was wrong to: rendering the same template through Jinja locally depends on `trim_blocks`/`lstrip_blocks`, which need not match what the server uses, and whitespace is tokens. The local renders are good for *relative* questions — does this branch differ from that one — and for reading what the template actually does. They are not calibrated against the backend, so "the numbers don't match, therefore an override is in force" was a conclusion the evidence could not carry. (It happened to be true; Juha said so independently.)
      - **Settled with the override removed: LM Studio does not forward the field, and the control is binary.** With the two prefix lines gone and 3.8 reloaded, `prompt_tokens` was **68 for absent, `medium`, `low` and `xhigh` alike, and 28 for `none`** — twice through, in that order. Had `medium` reached the template its block is 51 tokens shorter than `xhigh`'s, so the prompt would have been 17. It was 68. So on this backend the only client-side lever is thinking on or off; the gradation exists in the model and is unreachable over the API.
        - **The same run rehabilitates the local renders as an instrument, for differences.** Removing the override moved the prompt from 56 to 68 — the override had been pinning `low` — and the local stock renders put `xhigh` 12 tokens above `low` for the same message. The observed step is 12. So absolute counts are not comparable across the two renderers, and differences are.
      - **Flipping it mid-conversation is free on Qwen and costs the whole KV cache on Gemma** — measured 2026-08-26, `investigations/thinking-toggle-cache/`. The switch costs 0.06 s over a warm hit on `qwen3.6-35b-a3b` and a full re-prefill (1.57 s against a 0.20 s warm baseline, on 10k tokens) on `gemma4-26b-a4b`, because Qwen's thinking marker sits at the generation prompt and Gemma's at the top of the first system turn. Nothing to handle in code — it is a property of the template, unreachable from the client, and the direction the toggle needs works on both. Worth knowing before a demo toggles it on a long chat.
      - **What this means for the toggle**: on LM Studio, *Enable thinking* is `reasoning_effort: "none"` and nothing else — no prefill, no per-family marker table, no collision with `continue_`. The prefill stays the fallback for backends that do not implement it.
      - **What it means for Qwen 3.8 in the meantime**: a server-side template override is the only way to ask for less than `xhigh`, which is what Juha's does. Worth knowing that pinning `low` was reportedly not enough to make it usable interactively, so `none` — i.e. the toggle itself — may be the practical answer for a chat rather than an effort setting.
      - **`"none"` is general, and it works by rendering the template's own non-thinking branch.** Measured across three models, same question, `prompt_tokens` and `reasoning_tokens` together:

        | model | thinking on | under `"none"` | reasoning tokens under `"none"` |
        |---|---|---|---|
        | `qwen3.6-35b-a3b` | 26 | 28 | 0 (was 257 / 219) |
        | `qwen3.8-27b@q4_k_xl` | 68 | 28 | 0 |
        | `gemma4-26b-a4b` | 32 | 29 | 0 (was 202) |

        Every one answered `391` correctly with no reasoning at all, so this is a real switch rather than a truncation. Two things in that table settle the mechanism. **The prompt moves in different directions per model** — up for 3.6, down for 3.8 and Gemma — which a spliced-in marker cannot do, a prefill being able only to add. And **the two Qwens land on the same 28 from 26 and 68 while Gemma lands on its own 29**, which is what "each template's own non-thinking branch" predicts and nothing else does. Gemma is the discriminator that makes this conclusive: its reasoning markers are `<|channel>thought` / `<channel|>`, so a Qwen-shaped `</think>` spliced into it would be both visible in the count and wrong for the model.
        - **Consequence: no per-family marker table is needed at all** on this backend. The template that knows the model's own spelling is the one doing the work.
      - **Watch out for an unquoted Jinja `set` in a template override.** `{%- set reasoning_effort = low %}` binds an *undefined* name, and the stock template's `reasoning_effort|default('xhigh')` treats undefined exactly as absent — so the override renders as valid Jinja, LM Studio's validator accepts it, and the model silently runs at maximum effort. Rendered locally against the stock 3.8 template: quoted `'low'` gives 81 tokens and the low branch, unquoted gives 93 and the `xhigh` branch, identical to no override at all. Worth knowing before concluding anything from a run made under one.
  - **Design revision, and it is now itself under revision.** The paragraph below is the July design: a per-call parameter on `invoke` that becomes a *prefill* on LM Studio. The 2026-08-26 finding above puts a plain request field back on the table for that backend — `reasoning_effort: "none"` — which would be simpler than the prefill in every way that matters, since it needs no per-family marker table and does not collide with `continue_`. Settle that before building either. The prefill stays the fallback: it works on any OpenAI-compatible backend by construction, because a trailing assistant message means the template emits no generation prompt, and that is a property of the template rather than of the server. Keep the flavor-aware mapping: prefill on LM Studio, presumably `chat_template_kwargs` on ooba — **ooba is untested**, and is not installed on the 16 GB machine at all, so that half needs the ooba upgrade first.
  ...
  - **Gemma 4 (26B-A4B) probed 2026-07-27; same backend answers, opposite history behaviour.** `chat_template_kwargs` ignored, `min_p` honoured, prefill works, Anthropic-endpoint thinking toggle works, streaming works — all as for Qwen. Two differences that matter:
    - **Its template retains prior reasoning; Qwen's discards it.** Same measurement, opposite result: prompt size went 44 → 653 tokens when a ~700-token `reasoning_content` was present in history (Qwen stayed at 38). No `preserve_thinking` involved — the Gemma template has no such key and simply keeps it. Since `serialize_history_for_wire` sends `reasoning_content` untouched, **this is live, not hypothetical**: on Gemma every stored reasoning trace is re-sent every turn.
...
- **[Low]** Have the chat clients adopt `indexer.open_document_store`. `app.py` and `minichat.py` each carry their own copy of the same six-argument `hybridir.setup` call, and `raven-indexer` deliberately did not touch them — the shared function was written to match, not to replace, so that the tool could land without a GUI test. Three copies is where they start to drift; the conversion is mechanical but wants the GUI exercised once.
...
- **[Medium]** Make the scaffold scriptable: a script should be able to do everything the chat clients can, without standing up a GUI or a REPL. Today `perform_throwaway_task` is the only scripting entry point and it is deliberately thin — no datastore, so no attachments, no branch, no retrieval. The two symptoms that keep surfacing are the same gap seen from different sides: an attached document cannot be folded into a throwaway task (`llmclient.serialize_history_for_wire` needs the datastore to resolve the sidecar), and every batch experiment ends up re-implementing a slice of `scaffold.ai_turn` by hand. Supporting attachments in throwaway tasks is the narrow fix; the better one is a scripting-facing surface over the scaffold, which would also cover the manual-test probes under `briefs/*/manual_tests/`.
  ...
  - **Two entry points, not one.** Surveying the twelve probes under `briefs/librarian-extension/manual_tests/` (2026-07-29) splits them three ways, and only two of the three want anything:
...
- **[Medium]** User-level settings: move the configs a *user* legitimately changes out of `config.py` into JSON, and give them settings dialogs. Today every one of them — LLM backend URL, model, docs directory, avatar knobs — is a Python source edit, which is why the tracked `config.py` files carry local overrides on every dev machine and have to be kept out of every commit by hand. Same gap the Visualizer has (the "Settings window: expose `gui_config` in the GUI" item above), so the two want a shared answer rather than two dialogs.
  ...
  - **This needs a degraded startup mode, and that is the part that will bite.** Librarian currently checks for the LLM backend at startup and bails if it is absent — perfectly reasonable when the only way to change the URL is to edit source and restart. Once the URL lives in a settings dialog, bailing means the user cannot reach the one control that would fix the problem: server down, server moved, laptop on a different network, and the app refuses to open far enough to be told so. So the connection failure has to become a state the GUI can *run in* — chat disabled with a clear reason, settings reachable, and a retry that does not require a restart.
    - **The Visualizer had exactly this, and it was fixed on 2026-09-11** — worth reading before doing the Librarian one, because it is the same shape at a smaller scale. With LLM cluster keywords configured and the backend down, the app exited 255 at startup, no window: the pipeline connected in its own module body and the GUI imports it at module level. The fix was to move the check to where a run *starts* and report it as an exception each frontend places where it has room. `raven.visualizer.importer._setup_llm_backend`, and `tests/test_importer_startup.py` pins the structural part.
...
- **[High]** Wake-word trigger for voice input. The exhibit case: a visitor speaking directly to Aria reads very differently from an operator typing on their behalf. Higher demo value than anything else in this section, and the only item here that changes *who is talking* rather than how well it is heard.
  ...
  - **The visual side is buildable with what THA3 already has**, with three pieces of work:
    ...
    - **animefx** can put the visible "\ | /" over the character's head. It currently fires only on emotion changes, and a wake trigger is not an emotion change — so the trigger surface wants generalizing from "emotion changed" to "named effect fired", which is small and pays for itself the next time any event should drive an effect.
