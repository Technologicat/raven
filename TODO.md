...
- **[Medium]** HybridIR: give documents a **title** field. Today a document has only `document_id` (the path relative to `docs_dir`) and its text, so there is nothing to show a user, nothing to hand a model deciding whether a document is worth fetching, and nothing to weight in retrieval. Titles are usually already present in the data and merely unparsed: a `raven-burstbib` record is a verbatim BibTeX entry, so `title`/`author`/`year` are structured fields (`raven.papers.bibtex` parses them); an arXiv download or a hand-curated stash carries author, year and title in the filename; otherwise the first non-trivial line serves as a pseudo-title. Two wins, and the second is the larger: a legible label wherever a document is named (the `list_consulted_documents` inject in `briefs/summer_2026_librarian_extension/10_rag-tool-surface-brief.md`, a future citation UI), and a field that can be **weighted** in search — a title match is a much stronger relevance signal than a body match, which is index-side work adjacent to brief 09's query-side levers.
