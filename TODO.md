...
- **[High]** Thinking toggle: let the user turn a thinking model's reasoning on and off. Researched 2026-07-27 — brief 02 closed the *transport* half of this item but none of the request-side half, so the remaining work is narrower and differently shaped than originally written:
  ...
  - **Probed against LM Studio 2026-07-27; the obvious mechanism does not work and the rejected one does.** Measured on `qwen3.5-9b` and `qwen3.6-35b-a3b`:
    ...
    - **Prefill works, and is the mechanism.** Prefilling the assistant turn with `<think>\n\n</think>\n\n` drops reasoning to zero while the answer stays correct (9B 747→0, 35B 502→0). That string is **byte-identical to the template's own non-thinking branch**, so this reproduces non-thinking mode exactly rather than approximating it — no off-distribution prompt, and no "pretend the model said this" concern.
  ...
  - **Gemma 4 is untested and will differ.** The prefill string is template-shaped; Gemma's reasoning markup is not Qwen's, so read its template before assuming the same trick transfers.
  - **Qwen 3.6 wants its thinking history kept, and we currently can't grant that.** The two templates differ on reasoning retention (read from the GGUFs, 2026-07-27):
    ...
    `preserve_thinking` is a `chat_template_kwargs` key, which LM Studio ignores — and prefill can't substitute, since this governs how *history* renders rather than how the next turn starts. So on our default backend, the full-thinking-history mode 3.6 was designed for is unreachable through the API. **Open question:** whether LM Studio's per-model config can set it, the way it evidently sets `enable_thinking`. If it can, that's a config knob worth documenting; if not, it's an argument for the Anthropic-compat or ooba path. Note the cost either way — retaining every turn's reasoning enlarges the prompt substantially, which interacts with the context-budgeting and KV-cache items.
...
- **[Medium]** Anthropic-compatible backend support. LM Studio already serves an Anthropic-compat `/v1/messages` endpoint, verified working 2026-07-27 (proper Anthropic-shaped response with `stop_reason` / `usage.cache_read_input_tokens`), including assistant prefill. Supporting the Anthropic message format in `llmclient` would therefore cost little to test locally, and would let Raven talk to Anthropic's own API as a backend. Worth it for the user base: Raven targets scientific research, and a meaningful slice of that community already works through the Anthropic API. Same reasoning as supporting both NVIDIA and AMD — breadth of options, not a preference.
