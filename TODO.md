...
- **[High]** Thinking toggle: let the user turn a thinking model's reasoning on and off. Researched 2026-07-27 — brief 02 closed the *transport* half of this item but none of the request-side half, so the remaining work is narrower and differently shaped than originally written:
  ...
  - **Re-probed 2026-08-26, and one of the July findings has to be read differently now.**
    ...
    - **`reasoning_effort` is honoured, and `"none"` is a working thinking-off switch** — measured on `qwen3.8-27b@q4_k_xl`, LM Studio **0.4.20 (build 1)**. This is the finding that may replace the prefill mechanism below, so it is the one to re-check first if any of this is revisited.
      ...
      - **The gradation half of that is not a clean measurement**, and the reason is worth keeping: the model under test carries a hand-edited template override. Counting the *stock* template's own renders with the model's own vocabulary gives 93 / 81 / 42 / 46 tokens for xhigh / low / medium / thinking-off, and the backend reported 56 and 28 — no stock render matches either, so the override was in force throughout. The `"none"` result survives that (it changed both the prompt and the output, so the field reached *something*); "the other values do nothing" does not, and wants re-running on a model with no override.
      - **Next probe, when a model can be swapped in:** repeat on `qwen3.6-35b-a3b`, whose template has no effort branch and no override. If `"none"` zeroes the reasoning there too, the switch is LM Studio's own and Raven can simply send it; if not, it is something the override enables and the prefill mechanism stays.
