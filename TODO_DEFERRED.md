...
- **Whether the vendored licences belong in the top-level expression — resolved 2026-08-03: no, ship their
  ...
  **`xdotwidget` being LGPL-3.0 is the finding that matters here**, and it is this whole item repeating one
  ...
  permissive. It is adapted from `Technologicat/xdottir` — Juha's own project, but itself derived from
...
- **`raven/vendor/IconsFontAwesome6.py`** — no licence header. Its own comments say it was generated by
  `juliettef/IconFontCppHeaders` from Font Awesome 6 metadata. Font Awesome splits its terms (fonts under SIL
  OFL 1.1, icons CC BY 4.0, code MIT) and the generator has its own licence, so establish which of those
  covers a generated codepoint header before writing anything down. This one may genuinely be uncopyrightable
  data, but "may be" is not a determination.
