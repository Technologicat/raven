...
- **Pick a QR encoder.** Neither `qrcode` nor `segno` is installed (checked 2026-08-14); adding one is
  fine. `segno` is pure Python with no dependencies, `qrcode` leans on Pillow, which we already ship.
  Remember the CI install list is hand-maintained, so a runtime dependency goes in `pyproject.toml` *and*
  in `.github/workflows/requirements-ci.txt` if any test reaches it. Shipping a pre-generated PNG is the
  option that needs no encoder at all — the URL is a constant — at the cost of a binary asset that has to
  be regenerated by hand if it ever changes.
