...
| Alt+Left / Alt+Right | back / forward — moved to `briefs/filedialog-navigation-history-brief.md` |
| Ctrl+Left / Ctrl+Right | the same, in the listing — moved out with it |
| mouse back / forward | the same, for the mice that have the buttons — moved out with it |
...
**Moved out on 2026-08-21 to `briefs/filedialog-navigation-history-brief.md`, unscheduled** (Juha).
It is a new capability rather than a gap in the keyboard, the FileDialog work has run a week, and this
brief closes without it. The design went across whole and unedited; nothing here supersedes it.
...
**Moved out rather than left unbuilt:** the navigation history — Alt+Left / Alt+Right and the mouse's back
...
5. **The navigation history** — **moved out on 2026-08-21** to
   `briefs/filedialog-navigation-history-brief.md`, unscheduled. It was always independent of 1–4, which
...
10. ~~**The help card wants a cosmetic pass**~~ — **built 2026-08-21**, see "Added 2026-08-21 (afternoon)"
    ...
    **What the pass is actually for: the card is a fixed height sized for its tallest configuration.**
    ...
    fixed hotkey list whose height was tuned once and is correct. So the fix belongs at the call site
    (Juha, 2026-08-21): `fdialog` passes a height it works out, rather than `HelpWindow` growing a policy.

    **Do not size it by counting rows.** Tried on paper and it does not survive contact: a cell that wraps
    to two lines costs a line, several already do (`Go as deep as this entry allows`, `...and back,
    carrying the name`, `Sort by name / date / type / size`, `...or jump by first letter`), and which ones
    wrap depends on the column widths the table computes. Counting entries predicted a 30 px difference
    between the two configurations measured above where the real one is about 77 px, so the residue *is*
    the wrapping rather than a constant to calibrate away.

    **So measure the built table.** The card is built lazily on first F1 and its content never changes
    afterwards, so one measurement per dialog is enough. `HelpWindow.show` runs `_render` (which builds the
    window hidden), then `reposition(_force=True)`, then `show_item` — and `guiutils.recenter_window` takes
    an `update_window_size` flag, so machinery for re-measuring a help window already exists and is worth
    reading before adding any. The open question is where a measurement can be taken: layout needs a
    rendered frame, and the window is hidden until the last step.

    **Any probe that renders the card must call `guiutils.bootup(font_size=...)` before `create_viewport`.**
    Without it everything draws in DPG's built-in font, whose metrics are wrong and whose glyph coverage is
    ASCII — which produced both a phantom em-dash bug and a slack figure eight times too large on
    2026-08-21.
