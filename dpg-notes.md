...
Why the two halves differ is not fully identified. The standing candidate is that it is an ImGui/GLFW limitation rather than a DPG one — that once the first *mapped* context has been destroyed, the backend's state is undefined (Juha's recollection, 2026-09-01, offered as a recollection rather than a measurement). It fits what was measured: the crash arrives in `ImGui_ImplGlfw_WindowFocusCallback` under GLFW's `processEvent`, which is a focus event delivered to a backend whose context is gone.

**The signature to recognize it by is a few seconds of hang and then a `SIGSEGV`**, which is worth knowing because the hang is the part you notice first and it looks like slow work rather than like a crash on the way.

"Context recreation is not reliably safe once real widgets have rendered" below has what was measured, and what a function-scoped mapped fixture cost before the rule was stated this way.
