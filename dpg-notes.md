...
- **Verify that your scroll was applied; do not assume it.** `set_y_scroll` is applied later by the render loop, and DPG clamps it to the content height *at that moment*. If the content is briefly shorter right then, you are clamped to the smaller maximum and the content then grows back, leaving the view short of where you asked with no error raised anywhere. Read the position back a frame later and re-issue if it fell short (a couple of attempts is enough).

  A delete-then-add swap is exactly what creates that window, so this bites hardest in the case that needs scrolling most: streaming text. Raven's chat view replaces the in-progress paragraph by deleting its widget and re-rendering, and the `dpg.mutex()` that would confine the pair to one frame is disabled there because holding it hangs the app — so the shorter-content interval is observable by construction. Measured once: a scroll commanded to 533 px landed at 481.

  **Do not read 52 px as the characteristic size.** The gap is the height of whatever was swapped out, and here a "paragraph" is however much the model chose to emit between newlines — which can run to many lines, or a screenful. So the shortfall is unbounded, and any fix shaped like "widen the tolerance until it stops happening" cannot work even in principle: there is no value that covers an arbitrarily long paragraph without also swallowing every deliberate scroll. Reading the position back is magnitude-independent, which is the reason to prefer it rather than merely a tidier reason.

  Note that the exposure is not a property of your own code. It scales with the newline discipline of whatever produced the text, which for an LLM frontend is a remote model you do not control: a well-behaved one breaks paragraphs often and every swap is small, while one that emits a wall of text makes every swap a screenful. Tool output is the case to worry about rather than the model's prose — it is machine-generated and under nobody's stylistic control. (In Raven a tool result arrives as a complete message rather than streamed text, so it takes the swap path where the whole streaming widget is replaced, which shrinks the container by more still. That path restores the reader's offset explicitly for the same reason.)
