...
- **Setting a value programmatically does not fire the widget's callback.** `dpg.set_value(item, x)` changes the value and calls nothing; a callback runs only for a change the *user* made. Measured 2026-08-13 on `add_combo` and `add_input_text` — both silent, both with the value genuinely changed afterwards.
  ...
  - **All of which holds only while the field is *inactive*. On a field that holds the caret, the write is undone.** ImGui keeps its own edit buffer for an active `InputText` and that buffer is authoritative: `set_value` appears to work — `get_value` immediately afterwards reports the new string — and then the next frame writes the old buffer back **and fires the edit callback while doing it**. So on an active field both rules above invert: the write does not take, and a callback fires anyway. Measured 2026-08-17; typing `abc`, writing `SETVALUE`, then typing `Z` yields `abcZ`.
    - **`focus_item` elsewhere is what releases the caret, and it does not happen on the calling frame.** Polling `is_item_active` once per frame after focusing a button gave `[1, 0, 0, 0, 0, 0]` — still active for the first frame, released from the second. **Treat that as one observation on an idle app, not as a frame count to code against**: `focus_item` queues a change that ImGui applies on its next NewFrame, so how many *rendered* frames that costs depends on what else is queued and where the vsyncs fall. A dance built on "wait one frame" is a race that passes on a quiet app and fails on a busy one.
      ...
      - **`raven.common.gui.utils.release_caret` is that wait**, and `set_input_text` builds its write on it. Reach for it too wherever something must happen only *after* a field has let go — not only before a write. Raven-librarian's chat graph claims the keyboard through it: claimed on the calling frame, the claim was revoked at once by the per-frame check that hands the keys back when the composer is clicked into, because a composer still letting go reads exactly like one just entered.
...
- **A cap on rate, not on effort.** A frame that has already spent its budget gets no sleep at all, so the app runs flat out whenever the work is heavy. That is what "twelve frames a second unless you cannot" means, and it is the deliberate half of the trade.
  - **What makes that safe is that the whole constellation is already framerate-compensated.** `raven.common.gui.animation.Animator.render_frame` reads `time.monotonic_ns()` once per frame and hands it to every animation, each of which derives its state from `(t - t0)` in seconds — so a frame that arrives late shows a *later* stage of the animation rather than the next one. Dropping frames costs smoothness and nothing else: nothing runs slow-motion, nothing falls behind, and no state is skipped. A toolkit whose animations stepped per frame could not make this trade, because there a throttle would visibly slow everything down — which is also the reason to keep a new animation on the clock rather than on a frame counter.
  - **This is the older half of the story, and it starts with the avatar rather than with the GUI.** `raven.server.modules.avatar.Animator.interpolate` — a *different* `Animator` from the GUI one above, and both spellings are written out here for that reason — is where Juha worked out how to hold an animation to wall-clock speed under a frame rate that varies, and that math is what became `raven.common.smoothvalue` — whose derivation, from Newton's law of cooling, is in the `fps_corrected_step` comments and worth reading.
  - **Three layers compensate, by three different formulations, and all three are right.** The principle is one — *ask the clock, never the frame counter* — and what differs is what each layer finds convenient to index by. Worth having in view together, because reading any one of them first makes the other two look like they are doing it wrong.
    ...
    - **The postprocessor recomputes `frame_no` from timestamps every frame rather than accumulating**, deliberately — frame number is linear in time, so there is no reason to carry an error forward. Its comments work out how long the float stays exact, which is a good deal longer than any stream.
...
**`is_item_focused` on a *modal* window is `False` even while one of its own children has the caret.** Measured 2026-09-24 on DearPyGui 2.3.1, live in Raven-librarian's attach dialog (a modal `FileDialog`), with the app focused and a per-frame trace: the dialog's find field reported `is_item_active` True, while `is_item_focused` on the dialog's window stayed False through every keypress. So "a top-level window is focused while any child holds the focus", which does hold for the non-modal windows it was measured on, does not extend to modals. A modal owns the keyboard for as long as it is shown, so ask `is_item_shown` there instead. (`FileDialog.has_keyboard` asked focus for both kinds from 2026-09-09, and every modal dialog showed no keyboard mark for two weeks.)
...
**The signature to recognize it by is a few seconds of hang and then a `SIGSEGV`.** Worth knowing because the hang is the part you notice first, and it looks like slow work rather than like a crash on the way.

"Context recreation is not reliably safe once real widgets have rendered" below has what was measured, including where the crash lands, and what a function-scoped mapped fixture cost before the rule was stated this way.
...
**The hazard that makes stand-ins fiddly: a real DPG call with no context does not raise, it segfaults.** So a call the stand-in fails to cover does not surface as a red test naming the missing method — it takes the whole pytest process down, and what you get is a `Fatal Python error` and a screen of extension-module names. Two shapes of it, both met on 2026-09-01 while writing the Visualizer tests:

- **A call the module makes only on a path the test does not expect to reach.** `annotation._render_worker` calls `split_frame` right after its guards; a test that defeats a guard by accident then walks into it. The fix is to make the stand-in *raise* for the calls that mark "we have gone too far", rather than delegating them.
- **A call made through a *different* module's `dpg` binding.** `info_panel.select_cluster_by_id` reaches `selection.keyboard_state_to_mode`, which reads `selection`'s own `dpg`. Patching the module under test is not enough; patch every module the call chain passes through. Standing in for both is also the better test, since the real modifier mapping then runs against the keys the test presses.

The tell, when it happens, is that the crash dump names no test. Re-run with `-x` and bisect by test if you have to, but the first thing to check is which module's `dpg` the failing path actually uses.

**Know the ceiling before writing one: "DPG runs headless" is narrower than it sounds** (measured 2026-08-03, on a machine *with* a display). Contexts, widgets, themes and item state all work with an unshown viewport. But `dpg.render_dearpygui_frame()` **aborts the process** — `SIGABRT` on the GLFW assertion `window != NULL` in `glfwWindowShouldClose`, not a catchable exception — so nothing that needs *layout* is reachable: no real scroll extents, no `get_y_scroll_max`, no hit-testing, no measured text sizes, and **no widget coordinates** — a widget's drawn position and size (`get_item_rect_min`, `get_item_rect_size`) come out of layout, and layout only happens against a mapped viewport.

**The one coordinate that *is* readable headless is a window's `get_item_pos`, because it answers a different question**: it reports the position that was *set*, not the one drawn. So it is the right way to assert where a module decided to put a window (`test_tooltip.py` does exactly this, faking the `get_item_rect_size` the placement is computed from), and the wrong way to ask where a window ended up — which is also why a window's own position cannot serve as the reference in a `gui` test, and a child item's `rect_min` is read instead.
