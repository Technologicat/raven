...
| `dpg-polygon-closing/` | How to close a stroked polygon outline: `draw_polygon` strokes an open path, appending the first point closes the outline but leaves its seam unjoined, and `draw_polyline(closed=True)` is the one that mitres. Why the resulting notch only shows once something is zoomed in |
| `dpg-markdown-decorations/` | Why an inline-code background vanishes on a help card: the decoration is sized from a laid-out read, a hidden widget has no metrics, so the quad is built with zero area. Nesting and `prose_columns` turn out to be irrelevant — a coincidence that cost two wrong hypotheses. Also why building Markdown before the first frame used to hang the process |
...
| `thinking-toggle-cache/` | What flipping a toggle mid-conversation costs the KV cache. The thinking one is free on Qwen and the whole cache on Gemma, decided by where each template puts its thinking marker — which the two models together prove and either alone would not. The **tool** toggles (Internet, Documents) cost a re-prefill of the whole conversation on any model, the declarations sitting in the system turn, though a backend holding two prefixes makes the second flip each way cheaper |
...
| `backend-fault-injection/` | Two instruments for making a server misbehave on cue. `faultproxy.py` fails an LLM turn on demand, so what Librarian does when the backend errors *while the user is elsewhere* is testable rather than waited for — it found a streaming message widget left on screen after its turn ended away from the view, visible for a second or two, below what a driven screenshot catches. `tcprelay.py` makes a server *vanish* instead: kill it and Raven-server is gone, run it again and it is back, with the real server and its models untouched. That one found Librarian stuck at full frame rate for the rest of a session after the server went down |
...
| `graph-font-atlas/` | Whether the chat graph's four-face font ladder is something DPG's font atlas minds. It is not: every rung measures, and a face loads in a tenth of a millisecond at any size up to 1024 px — so rasterization follows what gets drawn rather than the font's whole range, which retires an estimate made by assuming otherwise. Also what the probes cannot say, measuring not being rendering, and which of Raven's two font problems this is not about |
| `chatgraph-rebuild-cost/` | What one rebuild of the chat graph view costs, and what it scales with — the number of boxes drawn rather than the size of the forest, so the sibling window is bounded by legibility rather than by time. Also the reason the depth window can hide the very level the sibling window exists for, what wrapping labels by measured width costs, and how a figure recorded here went ten times stale in six days |
...
| `stray-role-header/` | Why some Librarian replies open with a bare `assistant`. One send starts two AI turns milliseconds apart, and the second's request ends on the first's empty assistant node, so the model writes its own role header. Identified from logs and the datastore; not yet reproduced on purpose or fixed. Also holds a probe that keeps every request, whole turn record and live stream |
| `chat-search-highlight/` | What re-rendering chat paragraphs with search highlights costs, and how to swap one in without the view moving. A highlight changes the layout in colour as much as in bold, since each split run is measured separately; a hidden build keeps its decorations only if shown before the renderer's worker runs; and a scroll correction has to be set one frame *before* the swap to land with it — which the height change, predicted exactly from the renderer's own line breaks, makes possible |
