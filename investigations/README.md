...
| `highdim-clustering/` | Whether the Visualizer's clusters should be found in the embedding space rather than in the 2D map — yes, measurably, and that part is settled. Which algorithm to use once they are, is not: k-means is reliably last, agglomerative average-linkage leads on two representative corpora of three, and HDBSCAN's `eom` silently collapses to two clusters on two of them. Mostly a lesson about the yardstick — every interesting difference here was an artifact of coverage, cluster count, sliver clusters, or `float16` caches until a control removed it. Also kills two items of brief 11 on the data: PCA preprocessing hurts separation, and unconditional outlier assignment undoes the whole gain |
