...
| `bibliography-dedup/` | What each of `raven-deduplicate`'s rules actually did to a given `.bib`, listed and grouped rather than totalled. The instrument that found both of its false merges, neither of which a cluster count could show — so it is re-run whenever a rule changes, not once |
