...
Three tiers, separated because they are not equally reliable and a single removal count hides that:
...
So A and B1 are removals and B2 is a hand-check list, which is the whole reason for tiering rather than
counting. A record this cannot rule out stays, on the same asymmetry the judge uses: a false keep costs a
reader one line, a false drop loses a study with nothing left behind to notice it by.
...
def tier_of(fields: dict) -> str | None:
    """Which removal tier a record's extracted fields put it in, or `None` for none of them."""
...
def load_extracted(path: pathlib.Path) -> dict[str, dict]:
    """The extracted fields per citekey. A later line supersedes an earlier one."""
    out = {}
...
def main() -> int:
    ...
    parser.add_argument("--extracted", default=None, help="the extraction JSONL (default: beside this)")
    ...
    parser.add_argument("--keep-uncorroborated", action="store_true",
                        help="also remove tier B2, which this refuses to do by default. Read "
                             "`uncorroborated.tsv` before reaching for it: that tier is where the "
    ...
    extracted = load_extracted(pathlib.Path(opts.extracted) if opts.extracted
                               else here / "extracted.jsonl")
    ...
    held = {key for key, tier in tiers.items() if tier == TIER_B2} - doomed
    ...
    for tier in (TIER_A, TIER_B1, TIER_B2):
    ...
    held_path = out_dir / "uncorroborated.tsv"
    ...
    print(f"wrote {held_path}  ({len(held)} held back — read these, they are the risky ones)")
    return 0
