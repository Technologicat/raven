...
it builds the history through Raven's own code (`scaffold._perform_injects` +
...
def build(question, docs_query, matches):
...
def main():
    ...
    for model in MODELS:
        ...
        got = ask(model, build("What is the specific energy consumption of the Vantaa-3 stack?",
        ...
        got = ask(model, build("What is the specific energy consumption of the Kelvin-7 stack?",
        ...
        got = ask(model, build("What is 2+2?", None, []))
        show("2+2", got, "OK  " if "4" in got["content"] else "CHECK")

        print("  [4] Date and time: is the injected clock believed?")
        got = ask(model, build("What is today's date, and what time is it now?", None, []))
