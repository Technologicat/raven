...
- Check the crop settings in your `animator.json`. The shipped default is `"crop": {"enabled": true, "left": 0.2, "top": 0.0, "right": 0.8, "bottom": 1.0}`, which trims 20% from each side horizontally. This matches THA3's canonical cowboy-shot framing (head to mid-thigh, with substantial empty space at the sides) and gives a noticeable performance win, because the server-side postprocessor then runs on ~60% of the pixels. **If your character fills more of the frame horizontally** — for example, if you framed tighter than the THA3 template, or the pose extends wider (arms out, very wide hair) — loosen the crop edges (move `left` toward 0, `right` toward 1) so the character isn't clipped during sway animation. Watch the live preview in the settings editor to calibrate.
