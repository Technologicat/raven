...
# zoom (see `raven.common.image.lanczos.mip_scale_for_zoom`). A 1 MP image at
# fit-zoom pulls full res; a multi-MP photo at the same fit-zoom pulls only a
# small level — so the slow GPU→host readback never moves levels finer than the
# pane can show. The GPU mip *chain* is computed regardless (see
# `_preload_one`); the cap only governs the readback, trading a little idle-time
# readback + RAM for not re-decoding the image on navigation.
