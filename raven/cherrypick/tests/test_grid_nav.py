"""Tests for raven.cherrypick.grid navigation arithmetic.

Covers `_resolve_nav_target`, the pure core of relative grid navigation. The
...
from raven.cherrypick.grid import _resolve_nav_target
...
def test_current_visible(current, delta, expected):
    visible = [0, 1, 2, 3, 4]
    assert _resolve_nav_target(visible, current, delta) == expected


def test_single_visible_item_clamps_either_way():
    assert _resolve_nav_target([5], 5, +1) == 5
    assert _resolve_nav_target([5], 5, -1) == 5
...
def test_empty_visible_returns_none(delta):
    assert _resolve_nav_target([], 0, delta) is None
...
def test_tagged_out_first_image_then_next_lands_on_replacement():
    ...
    """
    assert _resolve_nav_target([1, 2, 3, 4], 0, +1) == 1


def test_hidden_current_in_middle():
    # current 2 hidden; gap sits between 1 and 3.
    visible = [0, 1, 3, 4]
    assert _resolve_nav_target(visible, 2, +1) == 3   # item after the gap
    assert _resolve_nav_target(visible, 2, -1) == 1   # item before the gap


def test_hidden_current_multi_step():
    # current 2 hidden; gap between 1 and 4.
    visible = [0, 1, 4, 5]
    assert _resolve_nav_target(visible, 2, +2) == 5
    assert _resolve_nav_target(visible, 2, -2) == 0


def test_hidden_current_before_all_visible():
    # current 0 hidden, everything visible is after it.
    visible = [2, 3, 4]
    assert _resolve_nav_target(visible, 0, +1) == 2   # first item after the gap
    assert _resolve_nav_target(visible, 0, -1) == 2   # nothing before -> clamp to first


def test_hidden_current_after_all_visible():
    # current 5 hidden, everything visible is before it.
    visible = [0, 1, 2]
    assert _resolve_nav_target(visible, 5, +1) == 2   # nothing after -> clamp to last
    assert _resolve_nav_target(visible, 5, -1) == 2   # last item before the gap
