...
class Postprocessor:
    ...
    #
    # `crt` is the one filter that answers "in the world" and sits in the Display band anyway, and the
    # exception is instructive rather than sloppy. It draws the finest structure in the picture, and any
    # filter that resamples or blurs erases it — so what places it is not what it depicts but what would
    # survive. Where a filter *depicts* something and where it has to *run* can come apart, and when they
    # do, the position is a measurement rather than a taxonomy question. Its docstring carries the numbers.
    #
    ...
    def crt(self, image: torch.tensor, *,
        ...
        The avatar is a hologram, so the raster belongs to the character: where the beam is not
        writing, it emits no light, and the gaps between the scanlines are transparent rather than
        dark. That reading does not depend on where in the chain this runs - the frame stays RGBA
        all the way through and the backdrop is composited at the client end - which is what lets
        the filter sit late enough to survive.

        Late is where it has to sit. The raster is the highest-frequency structure in the picture,
        so anything upstream of it is fine and almost anything downstream of it is not: a resample
        averages it away, and `bloom` erases it a second way, by max-combining alpha with a blurred
