...
Layer 1 - Utilities:        chatutil.py (~1.6k), appstate.py (~510), cleanup.py (~300),
                            imagestore.py (~270), textfilestore.py (~190), chatgraph.py (~850)
...
- **`chatgraph.py`** — The chat forest as a renderable picture: a datastore and a `ViewState` in, an xdot `Graph` plus a table of what each graph node *means* out. Pure — no DearPyGui (the xdot package imports its widget lazily, so the shapes cost nothing), no mutation of the forest — which is what makes the layout testable at all, a position being uncheckable through a rendered widget. Focus-plus-context rather than the whole tree: the branch to HEAD is a vertical spine, a few siblings show either side of it at each level, and *everything* omitted becomes a clickable gap, because a node with no visible links has to mean the graph really ends there. Four gap kinds share one primitive — siblings beyond the window (click bisects the hidden run), ancestors beyond the depth limit, the subtree under an off-spine sibling, and other roots. Tool-result nodes collapse onto the round that asked for them, with the count read from the assistant's own `tool_calls` so an interrupted turn still reports what it requested. Rows align on the spine node, which is what keeps a new sibling from moving anything but its own row. Consumed by `chatgraph_panel`.
