...
- [Features](#features)
    - [Multiversal history](#multiversal-history)
        - [Why?](#why)
        - [Chat graph](#chat-graph)
        - [Notes](#notes)
    - [Searching the chat](#searching-the-chat)
    - [Document database](#document-database)
        - [Indexing from the command line: `raven-indexer`](#indexing-from-the-command-line-raven-indexer)
        - [Turning a folder of arXiv PDFs into a searchable database](#turning-a-folder-of-arxiv-pdfs-into-a-searchable-database)
            - [Refreshing a collection when papers get new versions](#refreshing-a-collection-when-papers-get-new-versions)
...
- [GUI walkthrough](#gui-walkthrough)
    ...
    - [Chat message actions](#chat-message-actions)
        - [What else a message can say about itself](#what-else-a-message-can-say-about-itself)
    - [Mode toggles](#mode-toggles)
    - [What the indicators say](#what-the-indicators-say)
    - [Keyboard reference](#keyboard-reference)
        - [Always live](#always-live)
        - [While no dialog is open](#while-no-dialog-is-open)
        - [Chat log](#chat-log)
        - [Search](#search)
        - [The marked message](#the-marked-message)
        - [Mode toggles](#mode-toggles-1)
        - [Audio input panel](#audio-input-panel)
        - [Chat graph](#chat-graph-1)
        - [Hidden debug keys](#hidden-debug-keys)
...
- [Configuration](#configuration)
    ...
    - [AI avatar](#ai-avatar)
        - [How a character is put together](#how-a-character-is-put-together)
- [Future vision](#future-vision)
- [Troubleshooting](#troubleshooting)
    - [The LLM backend is not answering](#the-llm-backend-is-not-answering)
...
The box at the very top, **…N more cards**, is the one exception to "under the current character card": it walks through the *other* cards, one per click, coming back around to the one you are on. Editing your system prompt, your character card or your user card starts a new card, and the chats you held under the old one stay under it — where nothing else names them, since the chat log shows one branch and the graph draws one card. So this is the only route back to them. Your avatar and voice stay as configured while you look, whichever card's chats are on screen; and the box wears the **HEAD** pill while the chat you are actually in is behind it, which is how you find your way home.
...
**The ring stays where you left it** when the keyboard goes elsewhere — stepping away to fix a search term should not cost you your place — but it goes pale while it waits, so you can tell the box the arrows would move from the box you were last looking at. It brightens again the moment the graph has the keys, and you carry on from where you stopped: **Tab** away to read something, **Tab** back, and the next arrow moves from the box you were on.
...
While the cursor is on a box, the zoom controls turn about *it* rather than about the middle of the view — zoom in, zoom out and 1:1 all leave it where it is on screen and change the scale around it, so the thing you are looking at stays under your eye. A box near the edge of the view is nudged inward as it grows, far enough to stay whole; it only goes back to the middle of the view once it is too big to fit at all. With the cursor away, or panned out of sight, the controls work from the centre of the view as before. Zooming out stops once the whole tree is on screen; there is nothing further out to see.

The last group in the graph's toolbar offers the same six steps to the pointer — to the first sibling, ten left, one left, one right, ten right, to the last — and the counter beside them says which sibling the cursor is on, of how many at that level. That count is the one thing the picture cannot show you: most of a wide level is behind **…N more** boxes, and no two of them need be on screen together. The buttons act on the cursor, so they wake when it is on a box and go quiet again when **Esc** puts it away.
...
Everything in the panel but the sliders can be driven from the keyboard, each control by its own letter, and opening it hands the panel the keyboard so those letters work straight away. **M** measures the room — usually what you came for, since the microphone you chose last time is still chosen — **A** and **S** flip the two checkboxes, **R** resets to the configured defaults, and **Esc** closes the panel. The keyboard mark parks on the **Measure the room** button, but that is only where the keyboard rests: buttons here are not pressed with Space or Enter, so the letters are the whole interface. **D** goes to the microphone chooser, which wears a blue mark while it has the arrow keys: **Up**, **Down**, **Home** and **End** then step through the microphones, and **Esc** comes back out of it. The keys apply only while one of the panel's own controls has the focus, so they do not disturb what you are typing in the chat.
...
| Key | Action |
|---|---|
| `F1` | Help card — turn the pages with `Left` / `Right` (`Home` / `End` for the ends) |
...
| `Ctrl+F3` | Go to the next match **anywhere in the chat graph**, not only on the branch you are reading. Shows the graph if the avatar has the panel |
| `Ctrl+Shift+F3` | ...the previous one |
