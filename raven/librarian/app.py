...
def ai_turn(docs_query: Optional[str]) -> None:  # TODO: implement continue mode
    ...
    def run_ai_turn(task_env: env) -> None:
        ...
        def _add_to_avatar_preprocess_queue(new_paragraph: str) -> None:
            new_paragraph = new_paragraph.strip()
            if not new_paragraph:
                return
            if new_paragraph.startswith("<tool_call>"):  # don't speak and subtitle tool call invocations generated by the LLM
                return
            new_paragraph = strip_markdown.strip_markdown(new_paragraph)  # remove formatting for TTS and subtitling
            if new_paragraph is None:
                return
            new_paragraph = new_paragraph.strip()  # once more, with feeling!
            if not new_paragraph:
                return
