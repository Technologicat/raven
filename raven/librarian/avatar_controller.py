...
def initialize(avatar_instance_id: str,
    ...
    avatar_controller_config.input_queue_task_manager.submit(avatar_preprocess_tts_task, env())
...
def avatar_preprocess_tts_task(task_env: env) -> None:
    ...
    """
    logger.info(f"avatar_preprocess_tts_task: instance {task_env.task_name}: TTS preprocessor starting")

    def strip_emoji(text: str) -> str:
        return emoji.get_emoji_regexp().sub(r"", text)

    def process_item(text: str) -> None:
        logger.info(f"avatar_preprocess_tts_task.process_item: instance {task_env.task_name}: text 0x{id(text)}: analyzing '{text}'")
        text = text.strip()
        if not text:
            logger.info(f"avatar_preprocess_tts_task.process_item: instance {task_env.task_name}: text 0x{id(text)}: Text is empty after pre-stripping leading/trailing whitespace. Skipping.")
            return
        if text.startswith("<tool_call>"):  # don't speak and subtitle tool call invocations generated by the LLM
            logger.info(f"avatar_preprocess_tts_task.process_item: instance {task_env.task_name}: text 0x{id(text)}: Text is a tool call invocation. Skipping.")
            return
        text = strip_markdown.strip_markdown(text)  # remove formatting for TTS and subtitling
        text = strip_emoji(text)
        if text is None:
            logger.info(f"avatar_preprocess_tts_task.process_item: instance {task_env.task_name}: text 0x{id(text)}: Text is `None` after stripping markdown and emoji. Skipping .")
            return
        text = text.strip()  # once more, with feeling!
        if not text:
            logger.info(f"avatar_preprocess_tts_task.process_item: instance {task_env.task_name}: text 0x{id(text)}: Text is empty after post-stripping leading/trailing whitespace. Skipping.")
            return
        ...
        def check_exit_and_print_reason() -> bool:  # True: exit, False: keep running
            if task_env.cancelled or not avatar_controller_config.gui_alive:
                ...
                logger.info(f"avatar_preprocess_tts_task.process_item: instance {task_env.task_name}: {reason}, exiting.")
                return True
            return False
        ...
        logger.info(f"avatar_preprocess_tts_task.process_item: instance {task_env.task_name}: text 0x{id(text)}: detected {len(lines)} line{plural_s}.")
        for lineno, line in enumerate(lines, start=1):
            ...
            logger.info(f"avatar_preprocess_tts_task.process_item: instance {task_env.task_name}: text 0x{id(text)}, line {lineno}: detected {len(sentences)} sentence{plural_s} on this line.")
            ...
            for sentenceno, sentence in enumerate(sentences, start=1):
                ...
                logger.info(f"avatar_preprocess_tts_task.process_item: instance {task_env.task_name}: text 0x{id(text)}, line {lineno}, sentence {sentenceno} (0x{id(sentence):x}): starting processing")
                ...
                if avatar_controller_config.subtitles_enabled:
                    if avatar_controller_config.translator_target_lang is not None:  # Call the AI translator on Raven-server
                        ...
                        subtitle = sentence
                    logger.info(f"avatar_preprocess_tts_task.process_item: instance {task_env.task_name}: text 0x{id(text)}, line {lineno}, sentence {sentenceno} (0x{id(sentence):x}): original: {sentence}")
                    logger.info(f"avatar_preprocess_tts_task.process_item: instance {task_env.task_name}: text 0x{id(text)}, line {lineno}, sentence {sentenceno} (0x{id(sentence):x}): subtitle: {subtitle}")
                else:
                    logger.info(f"avatar_preprocess_tts_task.process_item: instance {task_env.task_name}: text 0x{id(text)}, line {lineno}, sentence {sentenceno} (0x{id(sentence):x}): original: {sentence}")
                    logger.info(f"avatar_preprocess_tts_task.process_item: instance {task_env.task_name}: text 0x{id(text)}, line {lineno}, sentence {sentenceno} (0x{id(sentence):x}): subtitler is off.")
                    subtitle = None
                ...
                logger.info(f"avatar_preprocess_tts_task.process_item: instance {task_env.task_name}: text 0x{id(text)}, line {lineno}, sentence {sentenceno} (0x{id(sentence):x}): precomputing TTS audio and phoneme data")
                ...
                if prep is None:
                    logger.warning(f"avatar_preprocess_tts_task.process_item: instance {task_env.task_name}: text 0x{id(text)}, line {lineno}, sentence {sentenceno} (0x{id(sentence):x}): error during precomputing, skipping sentence")
                    continue

                logger.info(f"avatar_preprocess_tts_task.process_item: instance {task_env.task_name}: text 0x{id(text)}, line {lineno}, sentence {sentenceno} (0x{id(sentence):x}): processing done")
    ...
    try:
        while True:
            ...
            try:
                ...
                logger.error(f"avatar_preprocess_tts_task: {type(exc)}: {exc}")
                traceback.print_exc()
    finally:
        reason = "Cancelled" if task_env.cancelled else "App is shutting down"
        logger.info(f"avatar_preprocess_tts_task: instance {task_env.task_name}: {reason}, TTS preprocessor exiting.")
