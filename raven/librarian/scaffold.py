...
def _create_incomplete_ai_payload(llm_settings: env) -> dict:
    ...
    An empty message, marked incomplete so that a reader can tell it from a reply that genuinely said
    nothing. `store_partial` replaces it as the text streams in, and the finished payload replaces it again
    when the reply is done.

    A node still marked incomplete when a datastore is *read back* was interrupted — a live turn cannot
    span a restart — so the marker answers both "is this being written right now" (asked while the turn
    holds it) and "was this ever finished" (asked at load), without either needing state of its own.
    """
    payload = chatutil.create_payload(llm_settings=llm_settings,
                                      message=chatutil.create_chat_message(llm_settings=llm_settings,
                                                                           role="assistant",
                                                                           text="",
                                                                           add_persona=False))
    payload["generation_metadata"] = {"status": "incomplete"}
    return payload
...
def ai_turn(llm_settings: env,
    ...
    `on_llm_start`: 1-argument callable, taking the node ID of the assistant message this round will write.
                    The return value is ignored.

                    The node exists already, carrying an empty message marked incomplete, and is filled in
                    as the reply streams. So a frontend rendering the reply reads it from the datastore like
                    any other message, rather than being handed the text and having to hold it: a view
                    rebuilt mid-reply finds the words that have arrived, because they are in the tree.
    ...
    while True:  # LLM agent loop - interleave LLM responses, tool calls and tool call results, until the LLM is done (no more tool calls).
        ...
        # The node exists *before* the reply does, so that everything which reasons over the tree can see a
        # reply while it is being written: the descent that decides where a branch ends, the sibling count,
        # the linearization a view is built from. A reply that is not a node is invisible to all of them,
        # and every consequence of that invisibility has to be patched somewhere else.
        #
        # It is created empty and filled in as the text arrives (`store_partial` below). Nothing reaches the
        # disk until the app exits, so this costs no writes.
        if not continue_this_message:
            ai_message_node_id = datastore.create_node(payload=_create_incomplete_ai_payload(llm_settings),
                                                       parent_id=head_node_id)
        else:
        ...
        def store_partial(message: dict) -> None:
            ...
            payload["generation_metadata"] = {"status": "incomplete"}
            datastore.overwrite_active_revision(ai_message_node_id, payload)
        ...
        try:
            out = llmclient.invoke(settings=llm_settings,
                                   ...
                                   on_partial_message=(store_partial if not continue_this_message else None))
            ...
            # doubles as "retry". It goes into the node this round already made rather than a new one:
            # there is exactly one assistant message here whatever happened, and creating a second would
            # leave the empty one beside it.
            ...
            error_payload = chatutil.create_payload(
                llm_settings=llm_settings,
                message=chatutil.create_chat_message(llm_settings=llm_settings,
                                                     role="assistant",
                                                     text=error_text,
                                                     add_persona=use_character_card))
            # No `generation_metadata`: there was no generation, so the renderer shows no token-stats line —
            # which is how a message Raven authored has always appeared. Its absence also clears the
            # "incomplete" marker, this message being finished, such as it is.
            if not continue_this_message:
                datastore.overwrite_active_revision(ai_message_node_id, error_payload)
            else:
        ...
        if not continue_this_message:  # new message (usual case)
            # The node was created before the call and has been carrying the reply as it streamed; this
            # replaces the last partial with the finished payload, whose `generation_metadata` also clears
            # the "incomplete" marker.
            datastore.overwrite_active_revision(ai_message_node_id, create_ai_payload())
