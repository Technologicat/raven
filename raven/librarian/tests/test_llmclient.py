...
class TestContinuingAMessageKeepsWhatItAlreadySaid:
    """A continuation's stream carries only the new text, so the reply is what was there plus what arrived.

    The failure this pins is silent and destructive: `invoke`'s accumulators start empty, so a message
    assembled from them alone is the *tail* presented as the whole message, and the earlier text is gone
    from the store with nothing raised. Both channels, since both accumulators start the same way — a reply
    interrupted while the model was still thinking loses its trace as well as its text.
    """

    SEED = "1. Spring\n2. Summer"
    TAIL = "\n3. Autumn\n4. Winter"

    def _continue(self, monkeypatch, settings, deltas, *, seed=SEED, seed_reasoning=None, extra=()):
        """Continue an assistant message reading `seed`, with `deltas` arriving as the continuation."""
        message = {"role": "assistant", "content": [chatutil.text_content_part(seed)]}
        if seed_reasoning is not None:
            message["reasoning_content"] = seed_reasoning
        history = _history("list the seasons") + [message]
        _fake_stream(monkeypatch, [*deltas, *extra, "[DONE]"])
        return llmclient.invoke(settings, history, tools_enabled=False, continue_=True)

    def test_the_text_is_joined(self, monkeypatch, invoke_settings):
        out = self._continue(monkeypatch, invoke_settings,
                             [{"choices": [{"delta": {"content": self.TAIL}}]}])
        text = chatutil.content_to_text(out.data["content"])
        assert text == self.SEED + self.TAIL, text

    def test_the_streamed_tail_alone_is_not_the_answer(self, monkeypatch, invoke_settings):
        """The negative control. Without it a fixture whose seed is empty passes the test above vacuously."""
        out = self._continue(monkeypatch, invoke_settings,
                             [{"choices": [{"delta": {"content": self.TAIL}}]}])
        text = chatutil.content_to_text(out.data["content"])
        assert text != self.TAIL, "the seed is empty, so this fixture cannot tell a joined reply from a replaced one"
        assert self.SEED in text

    def test_the_thinking_trace_is_joined_too(self, monkeypatch, invoke_settings):
        """Interrupt a model mid-thought and continue: the trace it had written must survive the round."""
        out = self._continue(monkeypatch, invoke_settings,
                             [{"choices": [{"delta": {"reasoning_content": " and winter"}}]}],
                             seed_reasoning="The seasons are spring, summer")
        assert out.data["reasoning_content"] == "The seasons are spring, summer and winter"

    def test_an_ordinary_reply_is_not_seeded(self, monkeypatch, invoke_settings):
        """The discriminating case for the other direction: seeding unconditionally would pass every test above.

        With `continue_=False` the last message is the user's, and prepending *that* would put the question
        into the model's mouth.
        """
        _fake_stream(monkeypatch, [{"choices": [{"delta": {"content": "The seasons are four."}}]}, "[DONE]"])
        out = llmclient.invoke(invoke_settings, _history("list the seasons"), tools_enabled=False)
        assert chatutil.content_to_text(out.data["content"]) == "The seasons are four."

    def test_a_backend_with_a_continue_flag_of_its_own_is_left_alone(self, monkeypatch, invoke_settings):
        """ooba continues through a request field rather than by prefill, and worked before the seed existed.

        Whether it answers with the whole message or only its tail is unmeasured and there is no install on
        hand to ask, so the seed is gated on the mechanism: seeding a backend that already returns the whole
        message would double it, and betting a working path on an unmeasured guess is the wrong way round.
        `TODO_DEFERRED.md`'s ooba item carries the measurement that would lift this.
        """
        invoke_settings.backend_supports_continue = True
        out = self._continue(monkeypatch, invoke_settings,
                             [{"choices": [{"delta": {"content": self.TAIL}}]}])
        assert chatutil.content_to_text(out.data["content"]) == self.TAIL

    def test_the_gate_is_off_for_the_backend_the_seed_was_measured_on(self, invoke_settings):
        """The control for the test above: with both flavors seeded the same way it would pass vacuously."""
        assert invoke_settings.backend_supports_continue is False, "this fixture is not a prefill-continuing backend"

    def test_the_token_count_is_what_this_call_generated(self, monkeypatch, invoke_settings):
        """The seed was generated by an earlier call, which was billed for it and timed against it.

        Counting it again here would inflate the speed the GUI reports for the continuation, and would
        disagree with the backend's own `usage` on every turn where that arrives — the two paths have to
        answer about the same span.
        """
        invoke_settings.tokenizer = env(encode=lambda text: text.split())  # one "token" per whitespace-separated word
        out = self._continue(monkeypatch, invoke_settings,
                             [{"choices": [{"delta": {"content": " three four"}}]}],
                             seed="one two")
        assert out.n_tokens == 2, f"counted {out.n_tokens}; the two seed words are not this call's output"

    def test_a_stopping_string_still_leaves_the_old_text_alone(self, monkeypatch, invoke_settings):
        """A continuation cut short by a stopping string keeps the message it was continuing.

        The stopping-string path assembles the reply from a *chopped* copy of the accumulator rather than
        from the accumulator itself, so it is the one route to `build_message` that could be joined
        differently from the others — and a reply that ends at a stopping string reads perfectly whether or
        not the earlier text is still on it.
        """
        invoke_settings.stopping_strings = ["\nUser:"]
        out = self._continue(monkeypatch, invoke_settings,
