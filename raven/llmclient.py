...
def create_chat_message(settings: env,
                        ...
                        tool_calls: List[str] = None) -> Dict[str, str]:
    ...
    `tool_calls`: Tool call requests; an array of JSON strings generated by the LLM.
...
def remove_role_name_from_start_of_line(settings: env, role: str, content: str) -> str:
    persona = settings.role_names.get(role, None)
    if persona is None:
        return content
    _role_name_at_start_of_line = re.compile(f"^{persona}:\\s+", re.MULTILINE)
    content = re.sub(_role_name_at_start_of_line, r"", content)
    return content

def scrub(settings: env, message: str, thoughts_mode: str, add_ai_role_name: bool) -> str:
    ...
    #
    message = remove_role_name_from_start_of_line(settings=settings, role="assistant", content=message)
    ...
    try:
        with timer() as tim:
            for event in client.events():
                ...
                message = payload["choices"][0]["delta"]
...
def minimal_chat_client(backend_url):
    ...
    try:
        ...
        def chat_print_message(message_number: Optional[int], role: str, content: str) -> None:
            ...
            print(remove_role_name_from_start_of_line(settings=settings, role=role, content=content))
