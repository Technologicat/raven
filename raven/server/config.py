...
animator_defaults = {
    ...
    # Only used if upscale != 1.0. `UPSCALE_QUALITIES` in `raven/common/video/upscaler.py` is the
    # definitive list; this summary is here so the file reads on its own. "low": anime4k fast, acceptable
    # image quality; "high": anime4k slow, good image quality; "bilinear": lightning-fast, bad quality;
