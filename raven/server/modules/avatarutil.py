...
supported_cels = [
    ...
    "data1", "data2", "data3", "data4",   # scifi "data eyes"; optional, up to 4 cels. Controlled only programmatically, via `raven.client.api.avatar_set_overrides`. The "data1" cel blend controls the strength.
            ...
            if filename.endswith(ext):
                if filename.startswith(f"{stem}_"):
                    ...
                    if celname in supported_cels or celname.startswith("fx_"):
                        ...
                        logger.info(f"scan_addon_cels: Ignoring chat icon '{filename}' for '{image_file_name}'. (This is normal; it's for the LLM frontend, not for Raven-avatar.)")
                    else:
