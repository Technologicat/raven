...
class TestReadingADatasetFile:
    def test_a_dataset_of_an_unknown_version_is_refused(self, tmp_path):
    ...
    def test_a_dataset_of_the_supported_version_is_read(self, dataset):
    ...
    def test_the_dataset_records_where_it_came_from(self, dataset, dataset_file):
...
class TestTheClusterSortWhichIsWhatADataIdxMeans:
    def test_the_data_is_sorted_by_cluster(self, dataset):
    ...
    def test_entries_and_coordinates_are_permuted_together(self, dataset):
    ...
    def test_each_entry_knows_its_own_index(self, dataset):
    ...
    def test_entry_titles_are_normalized_for_searching(self, tmp_path):
    ...
    def test_the_cluster_block_boundaries_slice_the_clusters(self, dataset):
    ...
    def test_the_kdtree_is_built_over_the_sorted_data(self, dataset):
    ...
    def test_parsing_a_dataset_does_not_publish_it(self, dataset_file, monkeypatch):
...
class TestWhichDatapointsAreOnScreen:
    def test_nothing_is_visible_when_no_dataset_is_loaded(self, gui, monkeypatch):
    ...
    def test_only_the_datapoints_inside_the_viewport_are_visible(self, gui, dataset):
    ...
    def test_a_datapoint_exactly_on_the_edge_of_the_viewport_counts_as_visible(self, gui, dataset):
    ...
    def test_a_datapoint_outside_the_viewport_is_not_visible(self, gui, dataset):
    ...
    def test_the_dataset_can_be_named_explicitly(self, gui, dataset, monkeypatch):
...
class TestWhichDatapointsAreUnderTheMouse:
    def test_the_datapoint_under_the_mouse_is_found(self, dataset, brush):
        found = points_at_mouse(dataset, brush, (30.0, 300.0))
        assert [dataset.sorted_entries[i].title for i in found] == ["The outlier"]

    def test_a_dataset_smaller_than_the_neighbor_budget_can_be_queried(self, dataset, brush):
    ...
    def test_a_datapoint_beyond_the_brush_is_not_found(self, dataset, brush):
    ...
    def test_the_brush_is_a_radius_in_pixels_not_in_data_units(self, two_points_apart_in_y, brush):
    ...
    def test_the_same_point_is_out_of_reach_at_an_unsquashed_scale(self, two_points_apart_in_y, brush):
    ...
    def test_nothing_is_under_the_mouse_when_no_dataset_is_loaded(self, brush, monkeypatch):
        monkeypatch.setattr(app_state, "dataset", None, raising=False)
        assert len(plotter.get_data_idxs_at_mouse()) == 0

    def test_a_degenerate_plot_scale_finds_nothing_rather_than_dividing_by_it(self, dataset, brush):
    ...
    def test_the_mouse_reported_at_exactly_the_origin_is_not_believed(self, point_at_the_origin, brush):
    ...
    def test_a_mouse_position_near_but_not_at_the_origin_is_believed(self, point_at_the_origin, brush):
...
class TestTheSelectRadiusBrushOutline:
    def test_the_brush_outline_is_a_circle_on_screen(self):
    ...
    def test_the_brush_outline_is_an_ellipse_in_data_space_when_the_axes_differ(self):
    ...
    def test_the_brush_outline_is_a_circle_in_data_space_when_the_axes_agree(self):
    ...
    def test_the_brush_outline_is_centred_on_the_cursor(self):
    ...
    def test_a_bigger_brush_draws_a_bigger_outline(self):
        small = np.ptp(outline(radius_pixels=5.0)[:, 0])
        large = np.ptp(outline(radius_pixels=20.0)[:, 0])
        assert large > small

    def test_the_brush_outline_is_a_closed_loop(self):
...
class TestTheHighlightBrightnessCurve:
    def test_the_highlight_brightens_with_the_animation_channel(self):
    ...
    def test_the_highlight_is_monotonic_in_the_animation_channel(self):
    ...
    def test_a_larger_set_is_drawn_more_faintly_per_datapoint(self):
    ...
    def test_the_per_datapoint_alpha_stops_falling_once_the_set_is_large(self):
    ...
    def test_the_alpha_stays_inside_the_range_a_colour_channel_has(self):
    ...
    def test_the_dimmest_a_highlight_gets_is_still_visible(self):
...
class TestZoomAndLifecycle:
    def test_resetting_the_zoom_fits_both_axes(self, gui):
        plotter.reset_zoom()
        assert gui.fitted == ["axis0", "axis1"]

    def test_clearing_the_cluster_colour_themes_deletes_them_all(self, gui, monkeypatch):
