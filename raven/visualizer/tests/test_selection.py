...
class TestTheFourCombineModes:
    ...
    def test_intersect_keeps_what_the_two_have_in_common(self, gui):
        selection.update([0, 1, 2])
...
class TestWhenAModeDecidesNothingChanged:
    def test_replacing_a_selection_with_itself_changes_nothing(self, gui):
        selection.update([0, 1, 2])
    ...
    def test_adding_points_that_are_already_selected_changes_nothing(self, gui):
        selection.update([0, 1, 2])
        gui.info_panel.calls.clear()
        assert selection.update([1, 2], mode="add") is False
        assert not gui.info_panel.called

    def test_subtracting_points_that_are_not_selected_changes_nothing(self, gui):
        selection.update([0, 1, 2])
        gui.info_panel.calls.clear()
        assert selection.update([4, 5], mode="subtract") is False
        assert not gui.info_panel.called

    def test_intersecting_with_a_superset_changes_nothing(self, gui):
        selection.update([0, 1])
        gui.info_panel.calls.clear()
        assert selection.update([0, 1, 2, 3], mode="intersect") is False
        assert not gui.info_panel.called

    def test_force_refreshes_the_gui_even_when_the_selection_stands_still(self, gui):
...
class TestScrollAnchoring:
    def test_the_items_surviving_a_selection_change_become_scroll_anchors(self, gui):
    ...
    def test_a_disjoint_selection_offers_no_anchors(self, gui):
        selection.update([0, 1])
...
class TestTheHighlightSeries:
    def test_the_highlight_gets_the_coordinates_of_the_selected_points(self, gui):
        selection.update([1, 3])
        xs, ys = gui.dpg.values[HIGHLIGHT_SERIES]
        assert sorted(xs) == [1.0, 3.0]
        assert sorted(ys) == [10.0, 30.0]

    def test_an_empty_selection_clears_the_highlight(self, gui):
        selection.update([1, 3])
        selection.update([])
    ...
    @pytest.mark.parametrize("mode, argument", [("replace", []),
                                                ("subtract", [0, 1, 2]),
                                                ("intersect", [])])
    def test_an_emptied_selection_is_still_an_index_array(self, gui, mode, argument):
...
class TestUndoHistory:
    def test_a_fresh_history_offers_neither_undo_nor_redo(self, gui):
        assert gui.dpg.enabled[UNDO_BUTTON] is False
        assert gui.dpg.enabled[REDO_BUTTON] is False

    def test_undoing_at_the_start_of_history_does_nothing(self, gui):
        selection.undo()
    ...
    def test_a_selection_change_offers_an_undo_but_not_yet_a_redo(self, gui):
        selection.update([0, 1])
        assert gui.dpg.enabled[UNDO_BUTTON] is True
        assert gui.dpg.enabled[REDO_BUTTON] is False

    def test_undo_restores_exactly_the_previous_selection(self, gui):
        selection.update([0, 1, 2])
        selection.update([3, 4], mode="replace")
        selection.undo()
        assert current_selection() == {0, 1, 2}

    def test_redo_puts_back_what_undo_took(self, gui):
        selection.update([0, 1, 2])
        selection.update([3, 4], mode="replace")
        selection.undo()
        selection.redo()
    ...
    def test_walking_back_to_the_start_disables_undo(self, gui):
        selection.update([0, 1])
        selection.undo()
        assert current_selection() == set()
        assert gui.dpg.enabled[UNDO_BUTTON] is False
    ...
    def test_walking_forward_to_the_end_disables_redo(self, gui):
        selection.update([0, 1])
        selection.undo()
        selection.redo()
        assert gui.dpg.enabled[REDO_BUTTON] is False
    ...
    def test_redoing_at_the_end_of_history_does_nothing(self, gui):
        selection.update([0, 1])
        gui.info_panel.calls.clear()
        selection.redo()
    ...
    def test_selecting_something_new_after_an_undo_discards_the_redo_branch(self, gui):
        selection.update([0, 1])
    ...
    def test_undo_walks_back_through_several_steps(self, gui):
    ...
    def test_committing_an_unchanged_selection_is_a_no_op(self, gui):
        selection.update([0, 1])
    ...
    def test_an_uncommitted_selection_change_leaves_no_undo_step(self, gui):
    ...
    def test_committing_afterwards_makes_the_whole_drag_one_undo_step(self, gui):
    ...
    def test_resetting_the_history_clears_the_selection_and_both_buttons(self, gui):
...
class TestWhoGetsToldAndWhetherTheyAreAskedToWait:
    def test_a_selection_change_tells_the_info_panel_the_tooltip_and_the_word_cloud(self, gui):
        selection.update([0, 1])
        assert gui.info_panel.called
    ...
    def test_wait_is_passed_on_to_everything_the_selection_refreshes(self, gui):
    ...
    def test_an_ordinary_selection_change_asks_nobody_to_wait(self, gui):
    ...
    def test_undo_and_redo_always_ask_the_updaters_to_wait(self, gui):
