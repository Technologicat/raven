...
| `check_option_lists.py` | Whether a list of options defined in code still appears wherever prose enumerates it — the upscaler's `quality` settings and Librarian's hotkeys today. Prose cannot import, so those copies are hand-maintained and nothing else has an opinion about them; a value added to the code and not to the documentation is invisible to every reader who learns the options from the docs. The check is one-way, code into prose, because a README is allowed to list *more* than its source does: Librarian's help card is knowingly incomplete, the chat graph's keys not fitting on it. A name counts only inside backticks or quotes, since `low` and `high` occur incidentally in any paragraph. |
